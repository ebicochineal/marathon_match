...
public:
    ...
    vector<int> buildJunctions(int S, vector<int> cities, double junctionCost, double failureProbability) {
        ...
        vector< vector < pair<int, int> > > bp(4, vector < pair<int, int> >());
        for (int i = 0; i < S * S; ++i) {
            if (v[i] == 1) {
                int x = i % S;
                int y = i / S;
                if (x < S/2 - S / 10 && y < S/2 + S / 10) {
                    bp[0].emplace_back(x, y);
                }
                if (x >= S/2 + S / 10 && y < S/2 - S / 10) {
                    bp[1].emplace_back(x, y);
                }
                if (x < S/2 - S / 10 && y >= S/2 + S / 10) {
                    bp[2].emplace_back(x, y);
                }
                if (x >= S/2 + S / 10 && y >= S/2 + S / 10) {
                    bp[3].emplace_back(x, y);
                }
        ...
        double iscore = score;
        int cnt = 0;
        vector<int> bestppos = {-1, -1, -1, -1};
        
        
        for (int k = 0; k < 4; ++k) {
            score = iscore;
            while(this->sw.get_milli_time() < 2300 * (k+1)){
                vector<int> ps;
