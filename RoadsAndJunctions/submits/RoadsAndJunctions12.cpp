...
public:
    ...
    vector<int> buildJunctions(int S, vector<int> cities, double junctionCost, double failureProbability) {
        ...
        if (failureProbability > 0.1) { iscore -= junctionCost; }
        if (failureProbability > 0.3) { iscore -= junctionCost; }
        ...
        for (int k = 0; k < bestppos.size(); ++k) {
            ...
            if (bestppos[k] > -1) {
                ...
                // cerr << tscore << endl;
                if ((score - tscore) / junctionCost > 6 && failureProbability > 0.1) {
                ...
                if ((score - tscore) / junctionCost > 12 && failureProbability > 0.1) {
                    int x = bestppos[k] % this->S;
                    int y = bestppos[k] / this->S;
                ...
                if ((score - tscore) / junctionCost > 24 && failureProbability > 0.1) {
                    int x = bestppos[k] % this->S;
                    int y = bestppos[k] / this->S;
