...
public:
    ...
    vector<int> buildJunctions(int S, vector<int> cities, double junctionCost, double failureProbability) {
        ...
        vector< vector < pair<int, int> > > bp(9, vector < pair<int, int> >());
        for (int i = 0; i < S * S; ++i) {
            if (v[i] == 1) {
                int x = i % S;
                int y = i / S;
        ...
        double iscore = score;
        int cnt = 0;
        vector<int> bestppos(9, -1);
        
        
        for (int k = 0; k < 9; ++k) {
            score = iscore;
            while(this->sw.get_milli_time() < (9500/9) * (k+1)){
                vector<int> ps;
                ...
                double tscore = this->all_distance(g, tvps) + junctionCost * (1.0 + failureProbability);
                if (tscore < score) {
                    score = tscore;
                    bestppos[k] = aa;
                }
