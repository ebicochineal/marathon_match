...
public:
    ...
    vector<int> buildJunctionsA (int S, vector<int> cities, double junctionCost, double failureProbability) {
        ...
        if (this->NC > 50) { nx = (5126 * 28) / this->NC; }
        if (this->NC > 70) { nx = (5126 * 24) / this->NC; }
        if (this->NC > 90) { nx = (5126 * 20) / this->NC; }
        ...
        for (int k = 0; k < putpos.size(); ++k) {
            if (this->sw.get_milli_time() > 9900) { break; }
    ...
    vector<int> union_pos(vector<int> &a) {
        ...
        for (int i = 0; i < p.size(); ++i) {
            ...
            for (int j = i+1; j < p.size(); ++j) {
                int bx = p[j] % this->S;
                int by = p[j] / this->S;
                double d = this->tdistance(ax, ay, bx, by);
                if (d < 8) {
                    sx += bx;
                    sy += by;
                    cnt += 1;
