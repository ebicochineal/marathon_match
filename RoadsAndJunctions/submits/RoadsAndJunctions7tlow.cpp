...
public:
    ...
    vector<int> buildJunctions(int S, vector<int> cities, double junctionCost, double failureProbability) {
        ...
        this->cities = cities;
        this->init_cities();
        this->init_ctiies_graph_triangle();
        ...
        vector<bool> tig(this->NC, false);
        while(this->sw.get_milli_time() < 9000 && bestppos.size() < this->NC*2) {
            score = iscore;
            int ta = -1, tb = -1, tc = -1, taa = -1;
            
            for (int k = 0; k < 128; ++k) {
                vector<int> ps;
                int a = xrnd()%this->center_poss.size();
                int aa = this->center_poss[a].first;
                
                int ai = this->center_poss[a].second[0];
                int bi = this->center_poss[a].second[1];
                int ci = this->center_poss[a].second[2];
                
                if (tig[ai] + tig[bi] + tig[ci] > 1) { continue; }
        ...
        // for (auto &i : this->center_poss) {
        //     ret.emplace_back(i.first % this->S);
        //     ret.emplace_back(i.first / this->S);
        // }
    ...
    vector< pair< int, array<int, 3>> > center_poss;// pos indexs
    void init_ctiies_graph_triangle () {
        vector< vector<int> > vii(this->NC, vector<int>());
        
        vector< pair<int, int> > ip = this->ip_cities;
        vector< pair<int, int> > r;
        vector< pair<int, int> > s;// index pos
        s.emplace_back(ip[0]);
        ip[0].first = -1;
        int S = this->S;
        while (s.size() < ip.size()) {
            ...
            ip[jj].first = -1;
            vii[ii].emplace_back(ji);
            vii[ji].emplace_back(ii);
        }
        this->cities_graph = r;
        
        vector<int> vcities(this->NC, -1);
        for (auto &i : this->ip_cities) { vcities[i.first] = i.second; }
        
        
        // triangle center
        for (auto &i : this->ip_cities) {
            if (vii[i.first].size() < 2) { continue; }
            int a = i.first;
            int pa = vcities[a];
            for (int j = 0; j < vii[i.first].size(); ++j) {
                for (int k = j + 1; k < vii[i.first].size(); ++k) {
                    int b = vii[i.first][j];
                    int pb = vcities[b];
                    int c = vii[i.first][k];
                    int pc = vcities[c];
                    array<int, 3> ar = {a, b, c};;
                    this->center_poss.emplace_back(this->triangle_center_pos(pa, pb, pc), ar);
                }
            }
        }
    }
    ...
    // pos pos pos
    int triangle_center_pos (int pa, int pb, int pc) {
        int ax = pa % this->S;
        int ay = pa / this->S;
        int bx = pb % this->S;
        int by = pb / this->S;
        int cx = pc % this->S;
        int cy = pc / this->S;
        return ((ax + bx + cx) / 3) + ((ay + by + cy) / 3) * this->S;
    }
