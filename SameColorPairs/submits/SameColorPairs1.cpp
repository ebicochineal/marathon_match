...
public:
    ...
    vector<string> removePairs(vector<string> board) {
        ...
        // cerr << print("H :", board.size(), ", W :", board[0].size(), " Time :", sw.get_milli_time() * 0.001) << endl;
    ...
    void remove_cells () {
        ...
        this->ret.clear();
        vector< vector<int> > vvi = {{0, 1}, {0, -1}, {1, 0}, {-1, 0}};
        ...
        while (f) {
            ...
            for (int i = 0; i < H; ++i) {
                for (int j = 0; j < W; ++j) {
                    ...
                    int px = -1;
                    int py = -1;
                    if (mdy > -1 && this->rect_first_remove(j, i, mdy, board, px, py)) {
    ...
    inline bool rect_first_remove (const int ax, const int ay, const int mdy, const vector<string> &board, int &px, int &py) {
        for (int i = ay + 1; i <= mdy; ++i) {
            for (int j = 0; j < W; ++j) {
                if (board[i][j] == board[ay][ax] && this->rect_is_remove(ax, ay, j, i, board)) {
                    py = i;
                    px = j;
                    if (xrnd() % 8 < 1) { return true; }
                }
            }
        }
        if (py > -1) { return true; }
    ...
    inline bool rect_is_remove (const int ax, const int ay, const int bx, const int by, const vector<string> &board) {
        ...
        for (int i = ay; i <= by; ++i) {
            for (int j = minx; j <= maxx; ++j) {
                if (board[i][j] > -1 && board[i][j] != board[ay][ax]) {
                    return false;
                }
            }
        }
