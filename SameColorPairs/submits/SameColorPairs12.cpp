...
public:
    ...
    vector<string> removePairs(vector<string> board) {
        ...
        while (swtime < 2000 && this->bestret.size() < H * W / 2) {
        ...
        while (swtime < 6000 && this->bestret.size() < H * W / 2) {
            ...
            if (this->ret.size() > this->bestret.size()) {
                ...
                if (this->ret.size() > H*W/2 - (64 - rets.size())) {
        ...
        while (swtime < 9800 && this->bestret.size() < H * W / 2) {
            if (cnt % 100 == 0 || (swtime > 9800 && cnt % 20 == 0)) { swtime = sw.get_milli_time(); }
    ...
    bool rect_first_remove (const int ax, const int ay, const int mrx, const int mdy, const int mlx, const int muy, vector<string> &board) {
        const char c = board[ay][ax];
        int tlx = mlx;
        int trx = mrx;
        
        for (int i = mlx; i <= mrx; ++i) {
            if (i == ax) { continue; }
