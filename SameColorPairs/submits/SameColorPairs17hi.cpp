...
public:
    ...
    vector< array< array<int, 100>, 100> > board;
    
    CumulativeSum2DColors (array< array<int8_t, 100>, 100> board, int H, int W, int C) {
        for (int i = 0; i < C ; ++i) {
            this->board.emplace_back(array< array<int, 100>, 100>());
        }
        ...
        for (int i = 0; i < H ; ++i) {
            ...
            for (int j = 1; j < W ; ++j) {
                for (int k = 0; k < C ; ++k) {
                    if (board[i][j] != k) {
                        this->board[k][i][j] = this->board[k][i][j-1] + 1;
                    } else {
                        this->board[k][i][j] = this->board[k][i][j-1];
                    }
                }
            }
        }
        ...
        for (int i = 0; i < W ; ++i) {
            for (int j = 1; j < H ; ++j) {
                for (int k = 0; k < C ; ++k) {
                    this->board[k][j][i] += this->board[k][j-1][i];
                }
            }
        }
    }
    
    inline bool is_rect_zero (const int ay, const int ax, const int by, const int bx, const int c) {
...
public:
    ...
    vector<string> removePairs(vector<string> board) {
        ...
        vector<int> re = {H*W/8, H*W/8, H*W/16, H*W/24, H*W/32, H*W/40, 32, 32, 32, 32, 24};
        
        for (int n = 1; n < 11; ++n) {
            while (swtime < 980 * n && this->bestret.size() < H * W / 2) {
