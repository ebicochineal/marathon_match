...
public:
    ...
    MM () {
        ...
        this->rects = this->SA0(500, this->rects, 0.85);
        this->rects = this->SA1(3400, this->rects, 0.90);
        //this->rects = this->SA(50, this->rects);
        pdebug(this->all_score(this->rects));
        this->expand_score_up(this->rects, 500);
    ...
    vector<RectXYR> SA1 (int limit, vector<RectXYR> ret, float v) {
        StopWatch sw;
        ...
        int cnt = 0;
        int lcnt = 0;
        vector< pair<int, vector<RectXYR>> > bestrets;
        while ((saTimeCurrent = sw.get_milli_time()) < saTimeEnd) {
            
            {
                ...
                if (tmpscore > lastscore || FORCE_NEXT) {
                    ...
                    if (tmpscore > bestscore) {
                        ...
                        if (bestrets.size() < 4) {
                            bestrets.emplace_back(bestscore, bestret);
                        } else {
                            bestrets[3].first = bestscore;
                            bestrets[3].second = bestret;
                        }
                        sort(bestrets.begin(), bestrets.end(), [](const pair<int, vector<RectXYR> >& a, const pair<int, vector<RectXYR> >& b) { return a.first > b.first; });
        ...
        if (bestrets.size() > 0) {
            for (auto&& i : bestrets) {
                this->expand_score_up(i.second, 500);
                i.first = this->all_score(i.second);
            }
            
            
            sort(bestrets.begin(), bestrets.end(), [](const pair<int, vector<RectXYR> >& a, const pair<int, vector<RectXYR> >& b) { return a.first > b.first; });
            bestret = bestrets[0].second;
        }
