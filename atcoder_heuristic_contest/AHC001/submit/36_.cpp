...
public:
    ...
    MM () {
        ...
        this->rects = this->SA00(300, this->rects, 0.5);
        this->rects = this->SA00(300, this->rects, 0.75);
        this->rects = this->SA00(300, this->rects, 0.85);
        this->rects = this->SA0(3000, this->rects, 0.90);
        ...
        this->rects = this->SA32(4960-this->sw.get_milli_time(), this->rects);
        pdebug(this->sw.get_milli_time());
        this->output();
    }
    ...
    vector<RectXYR> SA0 (int limit, vector<RectXYR> ret, float v) {
        ...
        sort(bests.begin(), bests.end(), [](const pair< int, vector<RectXYR> >& a, const pair< int, vector<RectXYR> >& b) { return a.first > b.first; });
        bestret = this->SA34(30, bestret);
        bestscore = this->all_score(bestret);
        while (this->sw.get_milli_time() < 4800) {
            ret = this->SA34(30, bests[xrnd()%bests.size()].second);
