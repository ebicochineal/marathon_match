...
public:
    ...
    MM () {
        ...
        for (int i = 0; i < 120; ++i) {
            ...
            vector<RectXYR> rects = this->calc(this->rects);
            int tscore = this->all_score(rects);
            if (tscore > bestscore) {
                bestscore = tscore;
                bestrects = rects;
            }
    ...
    vector<RectXYR> calc (vector<RectXYR> rects) {
        StopWatch sw = StopWatch();
        while (sw.get_milli_time() < 10) {
    ...
    int all_score (vector<RectXYR>& rects) {
        double r = 0;
        for (auto&& i : rects) {
