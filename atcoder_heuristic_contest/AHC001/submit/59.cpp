...
public:
    ...
    MM () {
        ...
        auto arects = this->SA0(2400, 4600, 100, this->rects, 0.5, 1);
        arects = this->SA34(2500-this->sw.get_milli_time(), arects);
        auto brects = this->SA0(2400, 4600, 100, this->rects, 0.5, 1);
        brects = this->SA34(4960-this->sw.get_milli_time(), brects);
        
        
        
        this->rects = this->all_score(arects) > this->all_score(brects) ? arects : brects;
    ...
    vector<RectXYR> SA0 (int limit, int limit2, int limit3, vector<RectXYR> ret, float v1, float v2) {
        ...
        int cnt = 0;
        int lcnt = 0;
        const double startTemp   = 1.0 / this->N * 0.02;
