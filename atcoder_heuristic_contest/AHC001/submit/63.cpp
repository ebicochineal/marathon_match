...
public:
    ...
    MM () {
        ...
        this->rects = this->SA0(4800, 4800, 40, this->rects, 0.5, 1);
    ...
    vector<RectXYR> SA0 (int limit, int limit2, int limit3, vector<RectXYR> ret, float v1, float v2) {
        ...
        // vector< pair< double, vector<RectXYR> > > bests;
        ...
        while ((saTimeCurrent = sw.get_milli_time()) < saTimeEnd) {
            
            {
                ...
                if (tmpscore > lastscore || FORCE_NEXT) {
                    ...
                    if (tmpscore > bestscore) {
                        ...
                        cnt = 0;
                        // if (bests.size() > 32) {
                        //     sort(bests.begin(), bests.end(), [](const pair< double, vector<RectXYR> >& a, const pair< double, vector<RectXYR> >& b) { return a.first > b.first; });
                        //     bests[bests.size()-1] = pair< double, vector<RectXYR> >(bestscore, bestret);
                        // } else {
            ...
            
            {
                ...
                for (int i = 0; i < this->N; ++i) {
                    si.emplace_back(this->score(ret[i]), i);
                }
                sort(si.begin(), si.end(), [](const pair<double, int>& a, const pair<double, int>& b) { return a.first < b.first; });
                
                
                
                int x = si[xrnd()%d].second;
                vector<int> vv;
                vv.emplace_back(x);
                auto vvv = this->testindex(ret, x);
        ...
        //     }
        // }
        // pdebug((int)((bestscore-ascore)*1000000000));
