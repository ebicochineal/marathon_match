...
public:
        ...
        if (this->sw.get_milli_time() < 1800) {
            ...
            for (int j = 0; j < 3; ++j) {
                for (auto&& x : this->paths[a]) {
                    ...
                    this->hvcost[x] = max(this->hvcost[x], 1000 + d);
                    this->hvcost[x] = min(this->hvcost[x], 9000 - d);
    ...
    vector<Edge> vedge (array<int, 1740>& hvcost) {
        ...
        for (int y = 0; y < b; ++y) {
            for (int x = 0; x < a; ++x) {
                ...
                if (x > 0) {
                    cost += hvcost[this->hvpos(0, x-1, y)];
                } else {
                    cost += hvcost[this->hvpos(0, x, y)];
                }
                if (x < a-1) {
                    cost += hvcost[this->hvpos(0, x+1, y)];
                } else {
                ...
                if (x > 1) {
                    cost += hvcost[this->hvpos(0, x-2, y)]/2;
                } else {
                    cost += hvcost[this->hvpos(0, x, y)]/2;
                }
                if (x < a-2) {
                    cost += hvcost[this->hvpos(0, x+2, y)]/2;
                } else {
                ...
                if (x > 2) {
                    cost += hvcost[this->hvpos(0, x-3, y)]/4;
                } else {
                    cost += hvcost[this->hvpos(0, x, y)]/4;
                }
                if (x < a-3) {
                    cost += hvcost[this->hvpos(0, x+3, y)]/4;
                } else {
                ...
                if (x > 3) {
                    cost += hvcost[this->hvpos(0, x-4, y)]/8;
                } else {
                    cost += hvcost[this->hvpos(0, x, y)]/8;
                }
                if (x < a-4) {
                    cost += hvcost[this->hvpos(0, x+4, y)]/8;
                } else {
                ...
                r.emplace_back(Edge(x+y*n, x+1+y*n, cost/(1+2+1+0.5+0.25)));
                r.emplace_back(Edge(x+1+y*n, x+y*n, cost/(1+2+1+0.5+0.25)));
            }
        }
        // DU
        for (int y = 0; y < a; ++y) {
            for (int x = 0; x < b; ++x) {
                ...
                if (y > 0) {
                    cost += hvcost[this->hvpos(1, x, y-1)];
                } else {
                    cost += hvcost[this->hvpos(1, x, y)];
                }
                if (y < a-1) {
                    cost += hvcost[this->hvpos(1, x, y+1)];
                } else {
                ...
                if (y > 1) {
                    cost += hvcost[this->hvpos(1, x, y-2)]/2;
                } else {
                    cost += hvcost[this->hvpos(1, x, y)]/2;
                }
                if (y < a-2) {
                    cost += hvcost[this->hvpos(1, x, y+2)]/2;
                } else {
                ...
                if (y > 2) {
                    cost += hvcost[this->hvpos(1, x, y-3)]/4;
                } else {
                    cost += hvcost[this->hvpos(1, x, y)]/4;
                }
                if (y < a-3) {
                    cost += hvcost[this->hvpos(1, x, y+3)]/4;
                } else {
                ...
                if (y > 3) {
                    cost += hvcost[this->hvpos(1, x, y-3)]/8;
                } else {
                    cost += hvcost[this->hvpos(1, x, y)]/8;
                }
                if (y < a-4) {
                    cost += hvcost[this->hvpos(1, x, y+3)]/8;
                } else {
                ...
                r.emplace_back(Edge(x+y*n, x+(y+1)*n, cost/(1+2+1+0.5+0.25)));
                r.emplace_back(Edge(x+(y+1)*n, x+y*n, cost/(1+2+1+0.5+0.25)));
            }
        }
        return r;
    }
    ...
    double all_cost_score (array<int, 1740>& hvcost) {
        ...
        for (int i = 0; i < this->paths.size(); ++i) {
            ...
            if (v > 0 && d / v > 5) {
                score += (d / v-2.5) * (d / v-2.5);
            }
            // if (t < this->truecosts[i] * 0.95 || t > this->truecosts[i] * 1.05) {
