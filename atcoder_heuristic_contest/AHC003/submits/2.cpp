...
public:
    ...
    array< array<int, 29>, 30> hcost;
    array< array<int, 30>, 29> vcost;
    ...
    MM () {
        ...
        for (int y = 0; y < 30; ++y) {
            for (int x = 0; x < 29; ++x) {
                this->hvcost[this->hvpos(0, x, y)] = 5000;
            }
        }
        for (int y = 0; y < 29; ++y) {
            for (int x = 0; x < 30; ++x) {
                this->hvcost[this->hvpos(1, x, y)] = 5000;
            }
        }
    ...
    void input2 () {
        ...
        for (auto&& i : this->vh) { this->hcost[i.y][i.x] = av; }
        for (auto&& i : this->vv) { this->vcost[i.y][i.x] = av; }
        ...
        //         vector<int> t;
        //         for (int x = 0; x < 29; ++x) {
        //             t.emplace_back(this->hvcost[this->hvpos(0, x, y)]);
        //         }
        //         fdebug(t);
        //     }
        //     for (int y = 0; y < 29; ++y) {
        //         vector<int> t;
        //         for (int x = 0; x < 30; ++x) {
        //             t.emplace_back(this->hvcost[this->hvpos(1, x, y)]);
        //         }
        //         fdebug(t);
        //     }
    ...
    void output () {
        ...
        if (this->turn >= 100) {
            ...
            while (x != this->ex) {
                if (x < this->ex) {
                    this->vh.emplace_back(x, y);
                    path.emplace_back(this->hvpos(0, x, y));
                    s += "R";
                    x += 1;
                }
                if (x > this->ex) {
                    this->vh.emplace_back(x-1, y);
            ...
            while (y != this->ey) {
                if (y < this->ey) {
                    this->vv.emplace_back(x, y);
                    path.emplace_back(this->hvpos(1, x, y));
                    s += "D";
                    y += 1;
                }
                if (y > this->ey) {
                    this->vv.emplace_back(x, y-1);
