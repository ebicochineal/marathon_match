...
public:
    ...
    void output () {
        ...
        if (this->paths.size()>0) {
            ...
            int n = 2;
            if (this->turn > 400) { n = 4; }
            if (this->turn > 800) { n = 8; }
            int t = 0;
    ...
    vector<Edge> vedge (array<int, 1740>& hvcost) {
        ...
        for (int y = 0; y < b; ++y) {
            for (int x = 0; x < a; ++x) {
                ...
                if (x > 3) {
                    cost += hvcost[this->hvpos(0, x-4, y)]/4;
                } else {
                    cost += hvcost[this->hvpos(0, x, y)]/4;
                }
                if (x < a-4) {
                ...
                r.emplace_back(Edge(x+y*n, x+1+y*n, cost/(1+2+1+0.5+0.5)));
                r.emplace_back(Edge(x+1+y*n, x+y*n, cost/(1+2+1+0.5+0.5)));
            }
        }
        // DU
        for (int y = 0; y < a; ++y) {
            for (int x = 0; x < b; ++x) {
                ...
                if (y > 3) {
                    cost += hvcost[this->hvpos(1, x, y-4)]/4;
                } else {
                    cost += hvcost[this->hvpos(1, x, y)]/4;
                }
                if (y < a-4) {
                ...
                r.emplace_back(Edge(x+y*n, x+(y+1)*n, cost/(1+2+1+0.5+0.5)));
                r.emplace_back(Edge(x+(y+1)*n, x+y*n, cost/(1+2+1+0.5+0.5)));
            }
        }
        return r;
    }
