...
public:
    ...
    void output () {
        ...
        if (this->paths.size()>0) {
            ...
            if (this->outtype == 0) {
                ...
                for (auto&& k : this->paths.back()) {
                    int p = k;
                    int hv = 0;
                    if (p >= 870) { hv = 1; p -= 870; }
                    if (hv == 0) {
                        int y = p / 29;
                        us.emplace(this->hvpos(0, 0, y));
                    } else {
                        int x = p % 30;
                        us.emplace(this->hvpos(1, x, 0));
    ...
    vector<Edge> vedge (array<int, 1740>& hvcost) {
        ...
        for (int y = 0; y < b; ++y) {
            for (int x = 0; x < a; ++x) {
                ...
                if (x > 1) {
                    cost += hvcost[this->hvpos(0, x-2, y)];
                } else {
                    cost += hvcost[this->hvpos(0, x, y)];
                }
                if (x < a-2) {
                ...
                if (x > 2) {
                    cost += hvcost[this->hvpos(0, x-3, y)];
                } else {
                    cost += hvcost[this->hvpos(0, x, y)];
                }
                if (x < a-3) {
                ...
                if (x > 3) {
                    cost += hvcost[this->hvpos(0, x-4, y)];
                } else {
                    cost += hvcost[this->hvpos(0, x, y)];
                }
                if (x < a-4) {
                ...
                r.emplace_back(Edge(x+y*n, x+1+y*n, cost/(1+2+2+2+2)));
                r.emplace_back(Edge(x+1+y*n, x+y*n, cost/(1+2+2+2+2)));
            }
        }
        // DU
        for (int y = 0; y < a; ++y) {
            for (int x = 0; x < b; ++x) {
                ...
                if (y > 2) {
                    cost += hvcost[this->hvpos(1, x, y-3)];
                } else {
                    cost += hvcost[this->hvpos(1, x, y)];
                }
                if (y < a-3) {
                ...
                r.emplace_back(Edge(x+y*n, x+(y+1)*n, cost/(1+2+2+2+2)));
                r.emplace_back(Edge(x+(y+1)*n, x+y*n, cost/(1+2+2+2+2)));
            }
        }
        return r;
    }
    ...
    vector<Edge> vedge2 (array<int, 1740>& hvcost) {
        vector<Edge> r;
        int a = 29;
        int b = 30;
        int n = 30;
