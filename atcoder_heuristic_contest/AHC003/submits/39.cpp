...
public:
    ...
    void HC () {
        ...
        if (this->sw.get_milli_time() > 1000) { b1 = 2; }
        if (this->sw.get_milli_time() > 1500) { b1 = 1; }
        if (this->sw.get_milli_time() > 1800) { a1 = 1; b1 = 1; b2 = 2; }
    ...
    void output () {
        ...
        if (this->paths.size()>0) {
            ...
            // if (t < p * 0.95 || t > p * 1.05) {
                for (auto&& j : this->paths.back()) {
                    ...
                    this->hvcost[j] = min(this->hvcost[j], 7000);
                }
            // }
        }
        ...
        //         vector<int> t;
        //         for (int x = 0; x < 29; ++x) {
        //             t.emplace_back(this->hvcostcnt[this->hvpos(0, x, y)]);
        //         }
        //         fdebug(t);
        //     }
        //     for (int y = 0; y < 29; ++y) {
        //         vector<int> t;
        //         for (int x = 0; x < 30; ++x) {
        //             t.emplace_back(this->hvcostcnt[this->hvpos(1, x, y)]);
        //         }
        //         fdebug(t);
        //     }
        // }
    ...
    vector<Edge> vedge0 (array<int, 1740>& hvcost) {
        vector<Edge> r;
        int a = 29;
        int b = 30;
        int n = 30;
        ...
        int m = max(12 - this->turn * 0.01, 3.0);
        // RL
        for (int y = 0; y < b; ++y) {
            for (int x = 0; x < a; ++x) {
                ...
                for (int i = 0; i < m; ++i) {
                    ...
                    d += (1.0 / (i+2)) * 2;
                    if (x > i) {
                        cost += hvcost[this->hvpos(0, x-g, y)] / (i+2);
                    } else {
                        cost += hvcost[this->hvpos(0, x, y)] / (i+2);
                    }
                    if (x < a-g) {
                        cost += hvcost[this->hvpos(0, x+g, y)] / (i+2);
                    } else {
        ...
        for (int y = 0; y < a; ++y) {
            for (int x = 0; x < b; ++x) {
                ...
                for (int i = 0; i < m; ++i) {
                    ...
                    d += (1.0 / (i+2)) * 2;
                    if (y > i) {
                        cost += hvcost[this->hvpos(1, x, y-g)] / (i+2);
                    } else {
                        cost += hvcost[this->hvpos(1, x, y)] / (i+2);
                    }
                    if (y < a-g) {
                        cost += hvcost[this->hvpos(1, x, y+g)] / (i+2);
                    } else {
    ...
    vector<Edge> vedge (array<int, 1740>& hvcost) {
        ...
        int m = max(16 - this->turn * 0.01, 6.0);
        // RL
        for (int y = 0; y < b; ++y) {
            for (int x = 0; x < a; ++x) {
                int cost = hvcost[this->hvpos(0, x, y)];
                ...
                for (int i = 0; i < m/(x < 15 ? 2 : 1); ++i) {
                    int g = i + 1;
                    if (x > i) {
                        cost += hvcost[this->hvpos(0, x-g, y)] / (i*0.5+1);
                    } else {
                        break;
                    }
                    d += (1.0 / (i*0.5+1));
                }
                for (int i = 0; i < m/(x < 15 ? 1 : 2); ++i) {
                    int g = i + 1;
                    if (x < a-g) {
                        cost += hvcost[this->hvpos(0, x+g, y)] / (i*0.5+1);
                    } else {
        ...
        for (int y = 0; y < a; ++y) {
            for (int x = 0; x < b; ++x) {
                ...
                for (int i = 0; i < m/(y < 15 ? 2 : 1); ++i) {
                    int g = i + 1;
                    if (y > i) {
                        cost += hvcost[this->hvpos(1, x, y-g)] / (i*0.5+1);
                    } else {
                        break;
                    }
                    d += (1.0 / (i*0.5+1));
                }
                for (int i = 0; i < m/(y < 15 ? 1 : 2); ++i) {
                    int g = i + 1;
                    if (y < a-g) {
                        cost += hvcost[this->hvpos(1, x, y+g)] / (i*0.5+1);
                    } else {
