...
public:
    ...
    void HC () {
        ...
        if (this->sw.get_milli_time() > 1600) { b1 = 4; }
        if (this->sw.get_milli_time() > 1750) { b2 = 2; }
        if (this->sw.get_milli_time() > 1850) { a1 = 1; b1 = 4; b2 = 2; }
    ...
    void output () {
        ...
        if (this->turn % 2 == 0 && this->turn > 20 && this->sw.get_milli_time() < 1700) {
            double refscore = 0;
            this->hvcost = this->SA(this->hvcost, refscore, 256, 0.1, 0);
        }
    ...
    T SA (T v, double& refscore, int ecnt, double stemp = 256.0, double etemp = 0.0) {
        ...
        for (int ncnt = 0; ncnt < ecnt; ++ncnt) {
            ...
            }
            {
                ...
                int b = xrnd() % this->paths.size();
                int x = this->paths[b][xrnd() % this->paths[b].size()];
                int tmp = v[x];
                v[x] += xrnd()%2 ? 100 : -100;
                v[x] = max(v[x], 1000);
                v[x] = min(v[x], 9000);
