...
public:
    ...
    void output () {
        ...
        this->HC(1900);
        if (this->turn % 2 == 0 && this->turn > 20 && this->sw.get_milli_time() < 1700) {
            double refscore = 0;
        ...
        if (this->turn < 20 || (bb && this->turn < 120 && (zacnt > len * 0.60 || zbcnt > len * 0.60)) || (bb && this->turn < 160 && (zacnt > len * 0.80 || zbcnt > len * 0.80))) {
            ...
            vector<Edge> tv = this->vedge(this->hvcost);
            vector<Edge> v = tv;
            if (this->preve.size() > 0) {
                int n = min((int)this->preve.size(), 2);
                double d = 1;
                for (int i = 0; i < n; ++i) {
                    for (int j = 0; j < v.size(); ++j) {
                        v[j].cost += this->preve[this->preve.size()-1-i][j].cost / 5;
                    }
                    d += 0.2;
                }
                for (int j = 0; j < v.size(); ++j) {
    ...
    void apply_score (array<int, 1740>& hvcost, vector<int> indexs) {
        static bitset<1000> pathindex_bs;
        for (int i = 0; i < 1000; ++i) { pathindex_bs.reset(i); }
        for (auto&& index : indexs) {
            for (auto&& i : this->recalc_ep_to_pi[index]) {
