...
public:
    ...
    int sx, sy, ex, ey;
    int turn = 0;
    array<int, 1740> hvcostsum;
    array<int, 1740> hvcost;
    array<int, 1740> hvcostcnt;
    
    array< vector<int> , 1740> recalc_ep_to_pi;
    array<double, 1000> tmpscore;
    array<double, 1000> tmpscore2;
    MM () {
        this->sw = StopWatch();
        int v = 3000;
        ...
        for (int y = 0; y < 30; ++y) {
            for (int x = 0; x < 29; ++x) {
                ...
                this->hvcostsum[this->hvpos(0, x, y)] = 0;
            }
        }
        for (int y = 0; y < 29; ++y) {
            for (int x = 0; x < 30; ++x) {
                ...
                this->hvcostsum[this->hvpos(1, x, y)] = 0;
            }
        }
    ...
    void HC (int t1) {
        ...
        int v = 600 - this->turn/4;
        
        int a1 = 2;
        int a2 = 2;
        int b1 = 2;
        int b2 = 2;
        if (this->sw.get_milli_time() > t1) { return; }
        
        double bestscore = this->init_score(this->hvcost);
        ...
        for (int j = 0; j < a1; ++j) {
            for (auto&& x : this->paths[a]) {
                ...
                double score = this->diff_score(this->hvcost, x);
                if (score+bestscore < bestscore) {
                    bestscore = score+bestscore;
                    this->apply_score(this->hvcost, x);
                } else {
                    this->hvcost[x] = tmp;
                }
            }
    ...
    void output () {
        ...
        if (this->paths.size()>0) {
            int ind = this->paths.size()-1;
            for (auto&& i : this->paths.back()) {
                this->recalc_ep_to_pi[i].emplace_back(ind);
            }
            
            
            int p = this->truecosts.back();
            int av = p/this->paths.back().size();
            int n = 2 + this->turnd() * 4;
            int t = 0;
        ...
        this->HC(1900);
        if (this->turn >= 20 && this->sw.get_milli_time() < 1900) {
            // double refscore = 0;
            // this->hvcost = this->SA(this->hvcost, refscore, 512, 0.1, 0);
            int n = 128+64+16;
            if (this->sw.get_milli_time() > 1850) { n = 32; }
            double refscore = 0;
            this->hvcost = this->SA2(this->hvcost, refscore, n, 0.2+this->turnd()*0.1, 0);
        }
        // this->HC2(1900);
        this->update_cost2();
        ...
        bool bb = ab >= len * 0.6;
        if (this->turn < 20 || (bb && this->turn < 120 && (zacnt > len * 0.60 || zbcnt > len * 0.60)) || (bb && this->turn < 160 && (zacnt > len * 0.80 || zbcnt > len * 0.80))) {
            int acnt = 0;
            int bcnt = 0;
            {
                int px = x;
    ...
    vector<Edge> vedge (array<int, 1740>& hvcost) {
        ...
        int m = 10 - this->turnd() * 6;
        
        //int xx = this->turn > 160;
        int xx = 1;
        // LR
        for (int y = 0; y < b; ++y) {
            for (int x = 0; x < a; ++x) {
                ...
                double d = 1;
                double z = (this->hvcostcnt[this->hvpos(0, x, y)]/6);
                cost += hvcost[this->hvpos(0, x, y)] * z * z;
                d += z * z;
                ...
                for (int i = 0; i < m; ++i) {
                    ...
                    if (x > i) {
                        ...
                        if (u > 0) {
                            cost += hvcost[p] / (i+1) * this->costv(u);
                            d += (1.0 / (i+1) * this->costv(u));
                        }
                    } else {
                        break;
                    }
        ...
        for (int y = 0; y < a; ++y) {
            for (int x = 0; x < b; ++x) {
                ...
                double d = 1;
                double z = (this->hvcostcnt[this->hvpos(1, x, y)]/6);
                cost += hvcost[this->hvpos(1, x, y)] * z * z;
                d += z * z;
    ...
    double init_score (array<int, 1740>& hvcost) {
        double score = 0;
        for (int i = 0; i < this->paths.size(); ++i) {
            int t = 0;
            for (int j = 0; j < this->paths[i].size(); ++j) {
    ...
    double diff_score (array<int, 1740>& hvcost, int index) {
        double score = 0;
        
        for (auto&& i : this->recalc_ep_to_pi[index]) {
            score -= this->tmpscore[i];
    ...
    void apply_score (array<int, 1740>& hvcost, int index) {
        for (auto&& i : this->recalc_ep_to_pi[index]) {
            this->tmpscore[i] =  this->tmpscore2[i];
        }
    }
    
    double diff_score_v (array<int, 1740>& hvcost, vector<int>& indexs) {
        static bitset<1005> pathindex_bs;
        for (int i = 0; i < this->turn+1; ++i) { pathindex_bs.reset(i); }
        
        double score = 0;
        for (auto&& index : indexs) {
            for (auto&& i : this->recalc_ep_to_pi[index]) {
                if (pathindex_bs[i]) { continue; }
                score -= this->tmpscore[i];
    ...
    void apply_score_v (array<int, 1740>& hvcost, vector<int>& indexs) {
        static bitset<1005> pathindex_bs;
        for (int i = 0; i < this->turn+1; ++i) { pathindex_bs.reset(i); }
        for (auto&& index : indexs) {
            for (auto&& i : this->recalc_ep_to_pi[index]) {
    ...
    template<class T>
    T SA (T v, double& refscore, int ecnt, double stemp = 256.0, double etemp = 0.0) {
        if (this->paths.size() == 0) { return v; }
        const double sacnt = ecnt;
        T bestv = v;
        double bestscore = this->init_score(v);
        double lastscore = bestscore;
        const int64_t R = 1000000000;
        for (int ncnt = 0; ncnt < ecnt; ++ncnt) {
            {
                int a = xrnd() % this->paths.size();
                int x = this->paths[a][xrnd() % this->paths[a].size()];
                int tmp = v[x];
                v[x] += xrnd()%2 ? 150 : -150;
                v[x] = max(v[x], 1000);
                v[x] = min(v[x], 9000);
                ...
                double tmpscore = this->diff_score(v, x)+lastscore;
    ...
    template<class T>
    T SA2 (T v, double& refscore, int ecnt, double stemp = 256.0, double etemp = 0.0) {
        if (this->paths.size() == 0) { return v; }
        const double sacnt = ecnt;
        T bestv = v;
        double bestscore = this->init_score(v);
        ...
        for (int ncnt = 0; ncnt < ecnt; ++ncnt) {
            {
                ...
                if (hv == 0) {// h
                    if (ab == 0) {
                        for (int i = 0; i < b; ++i) {
                            indexs.emplace_back(this->hvpos(0, i, line));
                        }
                    } else {
                        for (int i = b; i < 29; ++i) {
                            indexs.emplace_back(this->hvpos(0, i, line));
                    ...
                    if (ab == 0) {
                        for (int i = 0; i < b; ++i) {
                            indexs.emplace_back(this->hvpos(1, line, i));
                        }
                    } else {
                        for (int i = b; i < 29; ++i) {
                            indexs.emplace_back(this->hvpos(1, line, i));
                ...
                for (int i = 0; i < indexs.size(); ++i) {
                    v[indexs[i]] += g;
                    v[indexs[i]] = max(v[indexs[i]], 1000);
                    v[indexs[i]] = min(v[indexs[i]], 9000);
                }
                
                double tmpscore = this->diff_score_v(v, indexs)+lastscore;
