...
public:
    ...
    void solve1 (vector<int>& bestret1, vector<e512pos>& bestret2, double n) {
        vector<int> ret1;
        vector<e512pos> ret2;
        ret2.emplace_back(400, 400);
        vector<int> e;
        
        static bitset<1000> ss;
        static bitset<1000> es;
        ss.reset();
        es.reset();
        ...
        e512pos o;
        o.x = 400 + (xrnd()%601-300);
        o.y = 400 + (xrnd()%601-300);
        ...
        while (scnt < 50) {
            ...
            for (int i = 0; i < 1000; ++i) {
                if (ss[i]) { continue; }
                int dist = distance(p, this->S[i]) + distance(o, this->E[i]) * n + distance(e512pos(400, 400), this->E[i]) * n * 0.8;
                if (dist < bestdist) {
                    bestdist = dist;
                    best = i;
                }
    ...
    void solve () {
        vector<int> bestret1a;
        vector<e512pos> bestret2a;
        vector<int> bestret1b;
        vector<e512pos> bestret2b;
        
        this->solve1(this->bestret1, this->bestret2, 0);
        double bestscore = sumscore(this->bestret2);
        int cnt = 0;
        
        bestret1a = this->bestret1;
        bestret2a = this->bestret2;
        bestret1b = this->bestret1;
        bestret2b = this->bestret2;
        
        while (this->sw.get_milli_time() < 1600) {
            for (int i = 0; i < 12; ++i) {
                vector<int> ret1;
                vector<e512pos> ret2;
                this->solve1(ret1, ret2, 0.35+i*0.0125);
                double score = sumscore(ret2);
                // ret2 = minimize_sa_fixedstep(ret2, score, 5126, 32, 0);
                if (score < bestscore) {
                    bestscore = score;
                    bestret1b = bestret1a;
                    bestret2b = bestret2a;
        ...
        int time = (1960 - this->sw.get_milli_time()) * 0.5;
        
        double refa = 0;
        double refb = 0;
        bestret2a = minimize_sa(bestret2a, refa, time, 32, 0);
        bestret2b = minimize_sa(bestret2b, refb, time, 32, 0);
        if (refa < refb) {
            this->bestret1 = bestret1a;
            this->bestret2 = bestret2a;
        } else {
            this->bestret1 = bestret1b;
            this->bestret2 = bestret2b;
        }
        pdebug(refa, refb);
