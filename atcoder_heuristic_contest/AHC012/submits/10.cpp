...
public:
        ...
        while (true) {
            {
                ...
                int cy = v[a].cy;
                if (a < v.size()/2) {
                    v[a].position(xrnd()%20001-10000, cy);
                } else {
    ...
    void solve () {
        ...
        for (int i = 0; i < k/2; ++i) {
            ...
            this->bestout[i].position((20000/(k/2))*i-10000, 0);
        }
        for (int i = 0; i < k/2; ++i) {
            this->bestout[i+k/2].position(0, (20000/(k/2))*i-10000);
        }
        this->bestout = this->maximize_sa(this->bestout, 2800, 0, 0);
