...
public:
    ...
    void solve (int turn) {
        ...
        {
            int tt = (100 - turn) * (turn < 25 ? 3 : 5) + 5;
            // if (this->sw.get_milli_time() > 1700) { tt /= 5; }
            for (int z = 0; z < tt; ++z) {
                for (int i = 0; i < 4; ++i) {
                    array<array<int, 10>, 10> g = move(this->collect(i, this->grid));
                    this->randomput5(min(turn+1, 99), g);
                    int bscore = 0;
                    for (int j = 0; j < 4; ++j) {
                        array<array<int, 10>, 10> g2 = move(this->collect(j, g));
                        this->randomput5(min(turn+2, 99), g2);
                        for (int k = 0; k < 4; ++k) {
                            array<array<int, 10>, 10> g3 = move(this->collect(k, g2));
