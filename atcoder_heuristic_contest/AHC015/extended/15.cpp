...
public:
    ...
    void randomput5 (int turn, array<array<int, 10>, 10>& g) {
        int cnt = 0;
        int t1 = 1 + xrnd() % (100-turn);
        int t2 = 1 + xrnd() % (100-turn);
        int t3 = 1 + xrnd() % (100-turn);
        int t4 = 1 + xrnd() % (100-turn);
        int t5 = 1 + xrnd() % (100-turn);
        for (int y = 0; y < 10; ++y) {
            for (int x = 0; x < 10; ++x) {
                if (g[y][x] == 0) {
                    cnt += 1;
                    if (cnt == t1 || cnt == t2 || cnt == t3 || cnt == t4 || cnt == t5) {
                        g[y][x] = this->va[turn];
                    }
                }
            }
    ...
    void solve (int turn) {
        ...
        if (turn < 50) { ttt = 4; }
        if (turn < 25) { ttt = 3; }
        static array<array<int, 10>, 10> g;
        static array<array<int, 10>, 10> g1;
        static array<array<int, 10>, 10> g2;
        static array<array<int, 10>, 10> g3;
        
        {
            if (this->sw.get_milli_time() > 1750) { ttt = 4; }
            if (this->sw.get_milli_time() > 1800) { ttt = 3; }
            if (this->sw.get_milli_time() > 1850) { ttt = 2; }
            if (this->sw.get_milli_time() > 1900) { ttt = 1; }
            int tt = (100 - turn) * ttt;
            if (this->sw.get_milli_time() > 1950) { tt = 0; }
            if (tt == 0) {
                for (int i = 0; i < 4; ++i) {
                    g = move(this->collect(i, this->grid));
                    this->randomput5(min(turn+1, 99), g);
                    int bscore = 0;
                    for (int j = 0; j < 4; ++j) {
                        g2 = move(this->collect(j, g));
                        int score = this->gscore(g2);
                        if (score > bscore) {
                            bscore = score;
                        }
                    }
                ...
                for (int i = 0; i < 4; ++i) {
                    ...
                    for (int z = 0; z < tt; ++z) {
                        g1 = g;
                        this->randomput5(min(turn+1, 99), g1);
                        int bscore = 0;
                        for (int j = 0; j < 4; ++j) {
                            g2 = move(this->collect(j, g1));
                            this->randomput5(min(turn+2, 99), g2);
                            for (int k = 0; k < 4; ++k) {
                                g3 = move(this->collect(k, g2));
