...
public:
    ...
    void solve (int turn) {
        ...
        {
            int tt = 2000;
            if (this->sw.get_milli_time() > 1600) { tt = 1000; }
            if (this->sw.get_milli_time() > 1800) { tt = 100; }
            if (turn < 50) { tt = 100; }
            for (int z = 0; z < tt; ++z) {
                for (int i = 0; i < 4; ++i) {
                    array<array<int, 10>, 10> g = this->grid;
                    g = this->collect(i, g);
                    this->randomput(turn, g);
                    int bscore = 0;
                    for (int j = 0; j < 4; ++j) {
                        array<array<int, 10>, 10> g2 = g;
                        this->randomput(min(turn+1, 99), g2);
        ...
        //                     this->randomput(min(turn+2, 99), g3);
