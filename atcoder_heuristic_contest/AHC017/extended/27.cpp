...
public:
    ...
    void update_remove (int n, vector<UVW>& uvw, int i, bool f = false) {
        this->edgecost[uvw[i].u][this->us[i]].second = uvw[i].w;
        this->edgecost[uvw[i].v][this->vs[i]].second = uvw[i].w;
    }
    
    
    void update_add (int n, vector<UVW>& uvw, int i, bool f = false) {
        this->edgecost[uvw[i].u][this->us[i]].second = 20000000;
        this->edgecost[uvw[i].v][this->vs[i]].second = 20000000;
    }
...
struct BM {
    int64_t score = 0;
    array<int64_t, 100> scores;
    vector< vector<int> > v;
    BM () {}
    BM (int d, int64_t score, array<int64_t, 100>& scores, vector< vector<int> >& v) {
        this->score = score;
        this->scores = scores;
        this->v = v;
    }
...
public:
    ...
    void solve () {
        ...
        v = this->minimize_sa(5, v, 5850-this->sw.get_milli_time(), s*2, 0);
    ...
    int64_t calc_score_d_add (int n, int d, int index) {
        ...
        for (int i = 0; i < n; ++i) {
            this->gs[d][i].update_add(N, this->uvw, index, true);
    ...
    int64_t calc_score_d_remove (int n, int d, int index) {
        ...
        for (int i = 0; i < n; ++i) {
            this->gs[d][i].update_remove(N, this->uvw, index, true);
