...
void zzzz (int K, int turn, int m, Members& members, Tasks& tasks) {
    for (int i = 0; i < K; ++i) {
        members.skills[m-1][i] = max(tasks.skills[members.tasks[m-1]][i]-(turn - members.turns[m-1]), members.skills[m-1][i]);
    }
}
...
public:
    ...
    void output () {
        ...
        for (int turn = 0; turn < 2000; ++turn) {
            ...
            if (this->tasks.possibletasks().size() < 80) {
                ...
                for (int z = 0; z < zz; ++z) {
                    ...
                    for (auto&& t : pt) {
                        ...
                        for (auto&& m : fm) {
                            ...
                            // score -= this->tasks.rdependency[i].size();
