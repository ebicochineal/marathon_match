...
void zzzz (int K, int turn, int m, Members& members, Tasks& tasks) {
    ...
    int score = 0;
    for (int i = 0; i < K; ++i) {
        score += max(tasks.skills[members.tasks[m-1]][i]-members.skills[m-1][i], 0);
    }
    
    if (d < score/2) {
        for (int i = 0; i < K; ++i) {
            members.skills[m-1][i] = max(tasks.skills[members.tasks[m-1]][i]-d/2, members.skills[m-1][i]);
        }
    } else {
        for (int i = 0; i < K; ++i) {
            members.skills[m-1][i] = max(tasks.skills[members.tasks[m-1]][i]-d, members.skills[m-1][i]);
        }
    }
}
