...
void zzzz (int K, int turn, int m, Members& members, Tasks& tasks) {
    int d = turn - members.turns[m-1]-1;
    
    
    members.results[m-1].emplace_back(members.tasks[m-1], turn-members.turns[m-1]-1);
    
    int bestscore = scores(K, m-1, members, tasks);
    for (int i = 0; i < 3000; ++i) {
        int a1 = xrnd() % K;
        int b1 = xrnd() % 61;
        int t1 = members.skills[m-1][a1];
        int a2 = xrnd() % K;
        int b2 = xrnd() % 61;
        int t2 = members.skills[m-1][a2];
        // int a3 = xrnd() % K;
        // int b3 = xrnd() % 61;
        // int t3 = members.skills[m-1][a3];
        
        members.skills[m-1][a1] = b1;
        members.skills[m-1][a2] = b2;
        // members.skills[m-1][a3] = b3;
        int score = scores(K, m-1, members, tasks);
        if (score <= bestscore) {
            bestscore = score;
        } else {
            members.skills[m-1][a1] = t1;
            members.skills[m-1][a2] = t2;
            // members.skills[m-1][a3] = t3;
        }
    }
...
public:
    ...
    void output () {
        ...
        for (int turn = 0; turn < 2000; ++turn) {
            ...
            if (this->tasks.possibletasks().size() < 60) {
