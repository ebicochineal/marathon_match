...
//     pdebug(n);
// }
// void zzzz (int time, int K, int turn, int m, Members& members, Tasks& tasks) {}
...
void sa (int turn, int K, int m, Members& members, Tasks& tasks, int ecnt, double stemp = 256.0, double etemp = 0.0) {
    ...
    for (int ncnt = 0; ncnt < ecnt; ++ncnt) {
        int a1 = xrnd() % K;
        int b1 = xrnd() % (turn < 100 ? 31 : 61);
        int t1 = members.skills[m][a1];
        members.skills[m][a1] = b1;
        // int a2 = xrnd() % K;
        // int b2 = xrnd() % (turn < 100 ? 31 : 61);;
...
public:
    ...
    void output () {
        ...
        for (int turn = 0; turn < 2000; ++turn) {
            ...
            if (turn < 80 || this->tasks.possibletasks().size() < this->members.free_menbers().size()*2) {
                ...
                for (int z = 0; z < zz*0.7; ++z) {
                    ...
                    for (auto&& t : pt) {
                        ...
                        for (auto&& m : fm) {
                            ...
                            // score /= turn < 200 ? 2 : 1;
                            score -= this->tasks.allrdependency[t].size() * 0.2;
                            if (score < bestscore) {
                                bestscore = score;
                                bm = m;
                                bt = t;
            ...
            for (int i = 0; i < r; ++i) {
                ...
                zzzz(this->sw.get_milli_time(), this->K, turn, m, this->members, this->tasks);
