...
struct Tasks {
    ...
    array<int, gmaxN> depth;
    void init_depth () {
        unordered_set<int> us;
        for (int i = 0; i < gmaxN; ++i) {
            if (this->dependency[i].size() == 0) {
                this->depth[i] = 0;
                us.emplace(i);
            }
        }
        ...
        while (us.size() > 0) {
            tus.clear();
            for (auto&& p : us) {
                for (auto&& i : this->rdependency[p]) {
                    tus.emplace(i);
                    this->depth[i] = max(this->depth[p]+1, this->depth[i]);
                }
            }
            us = tus;
        }
...
double scores (int K, int m, Members& members, Tasks& tasks) {
    double score = 0;
    for (auto&& i : members.results[m]) {
        ...
        score += max(abs(result)-1.75, 0.0);
    }
    return score;
}

vector<double> gsv;
double initscores (int K, int m, Members& members, Tasks& tasks) {
    gsv = vector<double>(members.results[m].size(), 0);
    
    double score = 0;
    for (int i = 0; i < members.results[m].size(); ++i) {
        const int t = members.results[m][i].first;
        double result = members.results[m][i].second;
        for (int j = 0; j < K; ++j) {
            result -= max(tasks.skills[t][j]-members.skills[m][j], 0);
        }
        gsv[i] = result;
        score += max(abs(gsv[i])-1.75, 0.0);
    }
    return score;
}

double dscores (int K, int m, Members& members, Tasks& tasks, int p, int prev, int next) {
    double score = 0;
    for (int i = 0; i < members.results[m].size(); ++i) {
        const int t = members.results[m][i].first;
        double s = gsv[i];
...
//     pdebug(n);
// }
// void memberupdate (int time, int K, int turn, int m, Members& members, Tasks& tasks) { members.tasks[m] = -1; }
...
public:
    ...
    array< array<int, gmaxM>, gmaxN> truematnm;
    void tupdate (int turn) {
        // for (int n = 0; n < this->N; ++n) {
        for (auto&& n : this->tasks.readytask()) {
            int cnt = 0;
    ...
    void solve () {
        ...
        this->tasks.init_depth();
        
        for (int turn = 0; turn < 2000; ++turn) {
            if (turn == 300) { this->tasks.init_allrdependency(this->N); }
