...
struct Tasks {
    ...
    int sum_rdependency (int x) {
        int r = 0;
        for (auto&& i : this->rdependency[x]) { if (this->status[i] == 0) { r += 1; } }
        return r > 0;
    }
    ...
    void output () {
        ...
        for (int turn = 0; turn < 2000; ++turn) {
            ...
            if (r == -1) {
                // for (int i = 0; i < this->M; ++i) {
                //     vector<int> v;
                //     for (auto&& j : this->members.skills[i]) {
                //         v.emplace_back(j);
                //     }
                //     fdebug(v);
                // }
            ...
            for (int i = 0; i < r; ++i) {
                ...
                if (turn - this->members.turns[m-1] == 1) {
                    
                    for (int k = 0; k < this->K; ++k) {
                        int t = max(this->tasks.skills[this->members.tasks[m-1]][k]-3, this->members.minskills[m-1][k]);
                        this->members.minskills[m-1][k] = t;
                    }
                    // for (int k = 0; k < this->K; ++k) {
                    //     int t = max(this->tasks.skills[this->members.tasks[m-1]][k], this->members.skills[m-1][k]);
                    //     this->members.skills[m-1][k] = t;
                    // }
                }
                {
                    for (int k = 0; k < this->K; ++k) {
                        int t = max(this->tasks.skills[this->members.tasks[m-1]][k]-(turn - this->members.turns[m-1]), this->members.skills[m-1][k]);
                        this->members.skills[m-1][k] = t;
                    }
                }
