...
public:
    ...
    void solve () {
        ...
        // auto g1 = this->test16_1(11);
        // this->bdebug(g1);
        // auto g2 = this->expand(g1);
        // this->bdebug(g1, g2);
        // auto g3 = this->expand(g2);
        // this->bdebug(g1, g2, g3);
        // auto g4 = this->expand(g3);
        // this->bdebug(g1, g2, g3, g4);
    ...
    void solve2 (int& turn, VegeGrid& vg, bitset<256>& hg, int& money, bitset<256>& goal, vector<Com>& ret) {
        ...
        for (int b = 0; b < 256; ++b) {
            ...
            int score = tvg.ends[b]-turn < 5 ? tvg.vals[b] * h : tvg.vals[b] * h;
            if (score > bestscore1) {
                bestscore1 = score;
                bestb1 = b;
            }
    ...
    int solve1 (int& turn, VegeGrid& vg, bitset<256>& hg, int& money, bitset<256>& goal, vector<Com>& ret) {
        vg.update_f(this->veges, turn);
        int h = hg.count();
        int hc = this->harvester_cost(h);
    ...
    void bdebug (bitset<256>& g1, bitset<256>& g2) {
        for (int y = 0; y < 16; ++y) {
            string s;
            for (int x = 0; x < 16; ++x) {
                s += g1[y*16+x] | g2[y*16+x] ? "*" : " ";
            }
            fdebug(s);
        }
    }
    
    void bdebug (bitset<256>& g1, bitset<256>& g2, bitset<256>& g3) {
        for (int y = 0; y < 16; ++y) {
            string s;
            for (int x = 0; x < 16; ++x) {
                s += g1[y*16+x] | g2[y*16+x] | g3[y*16+x] ? "*" : " ";
            }
            fdebug(s);
        }
    }
    
    void bdebug (bitset<256>& g1, bitset<256>& g2, bitset<256>& g3, bitset<256>& g4) {
        for (int y = 0; y < 16; ++y) {
            string s;
            for (int x = 0; x < 16; ++x) {
                s += g1[y*16+x] | g2[y*16+x] | g3[y*16+x] | g4[y*16+x] ? "*" : " ";
            }
            fdebug(s);
        }
    }
