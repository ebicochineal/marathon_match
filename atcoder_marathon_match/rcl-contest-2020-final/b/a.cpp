...
public:
    ...
    vector< vector<int> > xgrid;
    e512pos rb;
    MM () {
        this->sw = StopWatch();
        
        cin >> this->N >> this->P >> this->M;
        pdebug(this->N, this->P, this->M);
        for (int y = 0; y < this->N; ++y) {
            this->grid.emplace_back(vector<int>(this->N, -1));
            this->xgrid.emplace_back(vector<int>(this->N, 0));
        }
        ...
        int minscore0 = 10000;
        for (auto&& i : um) {
            if (i.second > maxscore0) {
                maxscore0 = i.second;
                max0 = i.first;
            }
            if (i.second > minscore0) {
                minscore0 = i.second;
                min0 = i.first;
            }
        }
        ...
        //         if (this->xgrid[y][x] == 1) {
        //             if (this->grid[y][x] == max0) {
        //                 for (int i = 0; i < 4; ++i) {
        //                     int py = y + dy[i];
        //                     int px = x + dx[i];
        //                     if (inSide(px, py, this->N, this->N) && this->grid[py][px] != max0 && this->xgrid[py][px] == 0 && this->rb.y != py && this->rb.x != px) {
        //                         this->xgrid[y][x] = 0;
        //                         this->xgrid[py][px] = 1;
        //                         this->M -= 1;
        //                         break;
        ...
        for (int i = 0; i < this->M; ++i) {
            ...
            int bestscore = score;
            for (int n = 0; n < 4; ++n) {
                int tscore = this->moveScore(n, ret);
                if (tscore > bestscore) {
                    best = n;
                    bestscore = tscore;
                }
            }
            if (best < 0) { best = xrnd()%4; }
            e512pos mrb = this->move(best);
            
            if (this->grid[mrb.y][mrb.x] > -1) {
                ret.emplace_back(this->grid[mrb.y][mrb.x]);
            }
            score = this->vScore(ret);
            //fdebug(rb);
            this->grid[mrb.y][mrb.x] = -1;
            ...
            if (i == 300) {
                for (int y = 1; y < this->N; ++y) {
                    for (int x = 0; x < this->N; ++x) {
                        if (this->xgrid[y][x] == 1 && this->xgrid[y-1][x] == 0 && !(e512pos(x, y-1) == this->rb)) {
                            this->M -= 1;
                            cout << print("P", y, x, y - 1, x) << endl;
                            this->xgrid[y][x] = 0;
                            this->xgrid[y-1][x] = 1;
        ...
        //     fdebug(i);
        // }
        // for (auto&& i : this->xgrid) {
        //     fdebug(i);
        // }
        //fdebug(ret);
        pdebug("S",score, this->vScore(ret));
    ...
    e512pos move (int n) {
        ...
        while (1) {
            int x = r.x + dx[n];
            int y = r.y + dy[n];
            if (inSide(x, y, this->N, this->N) && this->xgrid[y][x] < 1) {
                r.x += dx[n];
                r.y += dy[n];
            } else {
                break;
    ...
    int moveScore (int n, vector<int>& v) {
        e512pos p = this->move(n);
        return this->vScore(v, this->grid[p.y][p.x]);
    }
    
    
    int vScore (vector<int>& v, int g) {
        if (g > -1) { v.emplace_back(g); }
        int cnt = 0;
        int score = 0;
        int prev = -1;
