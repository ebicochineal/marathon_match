...
public:
    ...
    MM () {
        ...
        int bestscore = 0;
        int loopcnt = 0;
        while (this->sw.get_milli_time() < 10000) {
            loopcnt += 1;
            ...
            while (p > 0) {
                ...
                while (1) {
                    ...
                    for (int j = 0; j < this->works.size(); ++j) {
                        ...
                        int pc = max(0, w.c - (int)((wet - t) * (p/(1.7))));
                        if (wd < 24 && (wet) - (wd + w.d) < 64) { pc *= 2; }
    ...
    //     vector<int> wsts(this->works.size(), -1);
    ...
    //                         t = wsts[v[i]];
    //                     } else {
    //                         work& w2 = this->works[v[i+1]];
    //                         t = max(w2.l - this->workdist(w.x, w.y, w2) - w.d - this->workdist(px, py, w), w.l - this->workdist(px, py, w));
    //                         wsts[v[i]] = t;
    //                     }
    //                 } else {
    //                     if (wsts[v[i]] > -1) {
