...
class TopCoderTesterQueue(threading.Thread):
    ...
    def half_marathon_python(self):
        ...
        cmd = self.op.py + ' ' + 'judge.py' + ' ' + fin + ' ' + fout
    ...
    def half_marathon_java(self):
        ...
        err = outerr[1].decode('shift-jis').replace('\r\n', '\n').strip()
        return out + ' ' + err + ' ' + perr
    def half_marathon_interactive_java(self):
        ...
        err = outerr[1].decode('shift-jis').replace('\r\n', '\n').strip()
        return out + err
    def half_marathon_interactive_python(self):
        cmd = self.op.py + ' ' + 'tester.py' + ' -seed ' + str(self.n) + ' -command ' + '\"' + self.cmdpath + '\"'
        p = Popen(cmd, stdout=PIPE, stderr=PIPE, shell=True)
        outerr = p.communicate(timeout=self.op.timeout)
        out = outerr[0].decode('utf-8').replace('\r\n', '\n').strip()
        err = outerr[1].decode('shift-jis').replace('\r\n', '\n').strip()
        return out + ' ' + err
    def topcoder_marathon(self):
        cmd = []
    ...
    def run(self):
        ...
        try:
            ...
            if self.op.type == 'half_marathon_python' : out = self.half_marathon_python()
