...
public:
    ...
    int playSlots (int startingCoins, int maxTime, int noteTime, int numMachines) {
        ...
        // if (startingCoins < 300 || maxTime < 1000 || startingCoins / numMachines < 50) { return 0; }
        ...
        //         if (this->coin < t) { break; }
        //         int tmp = this->quickPlay(i, t);
        //         if (tmp > maxcoin) {
        //             maxcoin = tmp;
        //             index = i;
        //         }
        ...
        int tt = max(min(maxTime / numMachines / noteTime / 10, 30), 12);
        // if (maxTime / numMachines / noteTime < tt) { return 0; }
        tt = min(maxTime / numMachines / noteTime / 2, tt);
        ...
        for (int i = 0; i < numMachines; ++i) {
            ...
            if (rate > 3) { break; }
            // if (this->time < 100 && rate > 1.5) { break; }
        }
        ...
        if (this->coin < tcoin) {
            this->get_prev_max_index_rate(index, rate);
        }
        if (rate > 0.90) {
    ...
    inline bool sunion (string &a, string &b) {
        
        
        return false;
    }
    
    inline void vunion (vector<string> &v, string &s) {
        if (v.size() == 0) {
            v.emplace_back(s);
            return;
        }
    ...
    inline string resset_to_string (unordered_set<string> &s) {
        vector<string> v;
        for (auto &i : s) {
...
int main() {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        sum += (int)d;
        if (d < 1) { zerocnt += 1; }
        if (d < g_rawcoins) { lowcnt += 1; }
        cout << "Score:" << d << endl;
    }
    cout << "Sum:" << sum << endl;
    cout << "ZeroCnt:" << zerocnt << endl;
    cout << "LowCnt:" << lowcnt << endl;
    return 0;
}
