...
public:
    ...
    int playSlots (int startingCoins, int maxTime, int noteTime, int numMachines) {
        ...
        //if (startingCoins < 300 && maxTime < 300 && (noteTime > 3 || numMachines > 4)) { return 0; }
        
        
        for (int k = 0; k < 24; ++k) {
            this->get_max_index_rate2(index, rate);
            for (int i = 0; i < numMachines; ++i) {
                if (k > 3 &&  this->expected2[i] < 0.1) { continue; }
                if (k > 6 &&  this->expected2[i] < 0.2) { continue; }
                if (k > 9 &&  this->expected2[i] < 0.3) { continue; }
                if (k > 12 && this->expected2[i] < 0.4) { continue; }
                if (k > 15 && this->expected2[i] < 0.5) { continue; }
                if (k > 18 && this->expected2[i] < 0.6) { continue; }
                if (k > 21 && this->expected2[i] < 0.7) { continue; }
                if (k > 6 &&  this->expected2[i] < rate - 3) { continue; }
                if (k > 12 && this->expected2[i] < rate - 2.8) { continue; }
                if (k > 15 && this->expected2[i] < rate - 2.7) { continue; }
                if (k > 18 && this->expected2[i] < rate - 2.6) { continue; }
                if (k > 21 && this->expected2[i] < rate - 2.5) { continue; }
            ...
            this->get_max_index_rate2(index, rate);
            if (rate > 0.90 && k > 2) {
                this->quickPlay(index, (startingCoins / 64 / (11-noteTime)) * k / (numMachines * 0.25));
                
                if (rate > 2 && this->coin > startingCoins + 1000) {
                    this->quickPlay(index, this->time);
                }
                
            }
            if (rate < 1 && this->coin < startingCoins / 2 && k > 5) { break; }
            if (this->coin == 0) { break; }
...
int main() {
    ...
    for (int y = 0; y < 10; ++y) {
        ...
        //cout << i << " Score:" << dscore << " " << iscore<< endl;
        cout << "Sum:" << dsum << " " << isum << endl;
