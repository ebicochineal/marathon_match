...
public:
    ...
    int playSlots (int startingCoins, int maxTime, int noteTime, int numMachines) {
        ...
        int tt = max(min(maxTime / numMachines / noteTime / 12, 30), 12);
        ...
        for (int i = 0; i < numMachines; ++i) {
            ...
            // if (rate > 2 && noteTime > 8 && maxTime < 5000) { break; }
    ...
    inline void resset_to_string (int m) {
        for (int x = 0; x < 3; ++x) {
            ...
            vector<string> v;
            string t = "";
            for (int i = 0; i < 128; ++i) { v.emplace_back(union_string(this->wheels2_res[m][x])); }
            sort(v.begin(), v.end(), [](const string &a, const string &b) { return a.size() < b.size(); } );
            for (int i = 0; i < 16; ++i) { t += v[i]; }
            t += v[v.size()-1];
