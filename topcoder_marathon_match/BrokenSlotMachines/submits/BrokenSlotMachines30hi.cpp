...
public:
    ...
    void lo (int startingCoins, int maxTime, int noteTime, int numMachines) {
        int index;
        double rate;
        int tt = max(min(maxTime / numMachines / noteTime / 14, 24), 14);
        ...
        for (int i = 0; i < min(this->coin / 4, 256); ++i) {
            sum += this->quickPlay(index, 1);
            psum += this->quickPlay(pindex, 1);
        }
