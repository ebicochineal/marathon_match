...
public:
    ...
    int playSlots (int startingCoins, int maxTime, int noteTime, int numMachines) {
        ...
        if (maxTime < 500 && noteTime > 5) {
            this->lo(startingCoins, maxTime, noteTime, numMachines);
        } else {
    ...
    void lo (int startingCoins, int maxTime, int noteTime, int numMachines) {
        // int st;
        // int c = maxTime / numMachines / 2;
        ...
        //     if (this->coin > st + 200) {
        //         while (this->time > 0 && this->coin > st / 2) {
        //             this->quickPlay(m, 1);
        //         }
        //     }
        // }
        ...
        int index;
        double rate;
        int tt = max(min(maxTime / numMachines / noteTime / 6, 20), 8);
    ...
    inline void resset_to_string (int m) {
        for (int x = 0; x < 3; ++x) {
            ...
            for (int k = 0; k < 8; ++k) {
                v.clear();
                for (int i = 0; i < 32; ++i) { v.emplace_back(union_string(this->wheels2_res[m][x])); }
...
int main() {
    ...
    for (int y = 0; y < 100; ++y) {
        ...
        // }
        // cout << endl;
        cout << y << " Sum:" << dsum << " " << isum << "all:" << adsum << endl;
