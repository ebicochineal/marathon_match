...
public:
    ...
    vector<string> createPuzzle (int width, int height, vector<string> dict) {
        ...
        int tmod = 32;
        if (lcnt > 500) { tmod = 48; }
        if (lcnt > 1000) { tmod = 64; }
        if (lcnt > 2000) { tmod = 128; }
        bool f = false;
        if (lcnt < 250) { f = true; }
        while(ntime = this->sw.get_milli_time() < 9700) {
            if (ntime > 9000) { loop = 1; }
            if (ntime > 9500) { f = true; }
            if (ntime > 9200 && lcnt < 600) { f = true; }
            
            for (int k = 0; k < loop; ++k) {
                wpds.clear();
                us.clear();
                int px = xrnd() % this->width;
                int py = xrnd() % this->height;
                for (int i = 0; i < bestwpds.size(); ++i) {
                    if (bestwpds[i].p.x >= px && bestwpds[i].p.x < px+8 && bestwpds[i].p.y >= py && bestwpds[i].p.y < py+8) { continue; }
                    if (xrnd()%8==0) { continue; }
                    wpds.emplace_back(bestwpds[i]);
                    us.emplace(bestwpds[i].w);
                }
                vs.clear();
                for (auto &i : this->dict) {
    ...
    void finfo (vector< vector<char> > vvc, vector<wpd> wpds) {
