...
public:
    ...
    vector<string> createPuzzle (int width, int height, vector<string> dict) {
        ...
        while(ntime = this->sw.get_milli_time() < 9000 - (e > 800 ? 200 : 0) + (e < 100 ? 400 : 0) + (e < 200 ? 200 : 0)) {
        ...
        for (int y = 0; y < theight; ++y) {
            for (int x = 0; x < this->width; ++x) {
                if (ret[y][x] == '-' || ret[y][x] == w[0]) {
                    ...
                    if (acnt > 0 && bcnt > 0 && ecnt == 0 && this->test_h(ret, x, y, w.size()) && this->ret_score(ret) > -1) {
                        return;
                    } else {
        ...
        for (int y = 0; y < this->height; ++y) {
            for (int x = 0; x < twidth; ++x) {
                if (ret[y][x] == '-' || ret[y][x] == w[0]) {
                    ...
                    if (acnt > 0 && bcnt > 0 && ecnt == 0 && this->test_w(ret, x, y, w.size()) && this->ret_score(ret) > -1) {
                        return;
                    } else {
                        for (int a = 0; a < w.size(); ++a) { ret[y][x+a] = tmp[a]; }
                    }
