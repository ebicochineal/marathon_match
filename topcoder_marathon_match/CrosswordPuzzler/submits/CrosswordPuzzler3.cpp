...
public:
    ...
    vector<string> createPuzzle (int width, int height, vector<string> dict) {
        ...
        while(this->sw.get_milli_time() < 9000 - (e > 800 ? 200 : 0) + (e < 100 ? 700 : 0)) {
            ret.clear();
            for (int i = 0; i < this->height; ++i) {
                ret.emplace_back( vector<char>(this->width, '-') );
            }
    ...
    void test_put (vector< vector<char> > &ret, string w) {
        ...
        int twidth = this->width - w.size();
        int theight = this->height - w.size();
        ...
        for (int y = 0; y < theight; ++y) {
            for (int x = 0; x < this->width; ++x) {
                if (ret[y][x] == '-' || ret[y][x] == w[0]) {
                    ...
                    if (ecnt == 0 && this->test_h(ret, x, y, w.size()) && this->ret_score(ret) > -1) {
                        return;
                    } else {
        ...
        for (int y = 0; y < this->height; ++y) {
            for (int x = 0; x < twidth; ++x) {
                if (ret[y][x] == '-' || ret[y][x] == w[0]) {
                    ...
                    if (ecnt == 0 && this->test_w(ret, x, y, w.size()) && this->ret_score(ret) > -1) {
                        return;
                    } else {
    ...
    bool test_w (vector< vector<char> > &ret, int px, int py, int wlen) {
    ...
    bool test_h (vector< vector<char> > &ret, int px, int py, int wlen) {
        unordered_set<string> w;
