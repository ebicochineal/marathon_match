...
public:
    ...
    vector<string> createPuzzle (int width, int height, vector<string> dict) {
        ...
        while(time = this->sw.get_milli_time() < 9000 - (e > 800 ? 200 : 0) + (e < 100 ? 500 : 0) + (e < 200 ? 200 : 0)) {
            if (time < 1000 && lcnt > 50) { loop = 2; }
            if (time < 1000 && lcnt > 200) { loop = 4; }
            if (time < 1000 && lcnt > 500) { loop = 8; }
            if (time > 8000) { loop = 1; }
            if (time > 8000 && lcnt > 2000) { loop = 2; }
            if (time > 8000 && lcnt > 5000) { loop = 4; }
            ...
            for (int k = 0; k < loop; ++k) {
                ...
                sort(vs.begin(), min(vs.end(), vs.begin() + 32), [](const string &a, const string &b) { return a.size() > b.size(); });
    ...
    void test_put (vector< vector<char> > &ret, const string &w) {
    ...
    void test_put2 (vector< vector<char> > &ret, const string &w) {
    ...
    void test_putg (vector< vector<char> > &ret, const string &w) {
        ...
        for (int y = 0; y < theight; ++y) {
            for (int x = 0; x < this->width; ++x) {
                if (ret[y][x] == '-' || ret[y][x] == w[0]) {
                    ...
                    if (acnt > maxa && acnt > 1 && bcnt > 0 && ecnt == 0 && this->test_h(ret, x, y, w.size()) && this->ret_score(ret) > -1) {
                        maxa = acnt;
                        pos.x = x;
                        pos.y = y;
                        dir = 1;
        ...
        for (int y = 0; y < this->height; ++y) {
            for (int x = 0; x < twidth; ++x) {
                if (ret[y][x] == '-' || ret[y][x] == w[0]) {
                    ...
                    if (acnt > maxa && bcnt > 0 && ecnt == 0 && this->test_w(ret, x, y, w.size()) && this->ret_score(ret) > -1) {
                        maxa = acnt;
                        pos.x = x;
                        pos.y = y;
                        dir = 0;
