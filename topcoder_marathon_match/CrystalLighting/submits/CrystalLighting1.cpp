...
public:
    ...
    vector<string> placeItems(vector<string> targetBoard, int costLantern, int costMirror, int costObstacle, int maxMirrors, int maxObstacles) {
        ...
        int cnt = 0;
        int m = this->H * this->W > 5000 ? 4 : 5;
        int d = this->H * this->W > 5000 ? 1 : 5;
        // for (int i = 1; i < m; ++i) {
            while (this->sw.get_milli_time() < 6500) {
            ...
            while (this->sw.get_milli_time() < 7000) {
                vector<clitem> ret;
        ...
        for (int k = 0; k < 2; ++k) {
            ...
            for (int i = 0; i < this->H; ++i) {
                if (i % 10 == 0 && this->sw.get_milli_time() > 9700) { break; }
                for (int j = 0; j < this->W; ++j) {
                    vector<clitem> ret = bestret;
                    if (b[i][j] != '.' || mo < 1) { continue; }
                    ret.emplace_back(i, j, 'X');
            ...
            for (int z = 0; z < 10; ++z) {
                int mcnt = 0;
                for (auto &i : bestret) {
                    if (mcnt % 50 == 0 && this->sw.get_milli_time() > 9700) { break; }
                    vector<clitem> tret;
        ...
        //cerr << print("Score", this->all_score(bestret), "Time", this->sw.get_milli_time(), "cnt", cnt, "retcnt", bestret.size()) << endl;
