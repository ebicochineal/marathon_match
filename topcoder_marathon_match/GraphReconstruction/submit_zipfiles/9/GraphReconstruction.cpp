...
public:
    ...
    vector<string> findSolution (int N, double C, int K, vector<string> paths) {
        ...
        sort(this->paths.begin(), this->paths.end(), [](const ABC& a, const ABC& b) { return a.a < b.a; });
        ...
        //for (int i = 0; i < 8; ++i) { this->disConnect(); }
        // auto e = this->initEdges();
        ...
        for (int i = 0; i < 8; ++i) { this->disConnect(); }
        // for (int i = 0; i < 8; ++i) { this->disConnect2(); }
        this->nearConnect0(100, 10);
        this->nearConnect0(100, 6);
        this->nearConnect0(2, 4);
        this->nearConnect0(4, 4);
        this->nearConnect0(8, 4);
        this->nearConnect0(32, 4);
        this->nearConnect0(100, 4);
        this->nearConnect0(2, 2);
        this->nearConnect0(4, 2);
        this->nearConnect0(8, 2);
        this->nearConnect0(32, 2);
        this->nearConnect0(100, 2);
        this->nearConnect(true);
        this->nearConnect(false);
    ...
    void nearConnect0 (int p, int o) {
        ...
        for (int i = 0; i < this->N; ++i) {
            for (int j = 0; j < this->N; ++j) {
                ...
                for (int k = 0; k < this->N; ++k) {
                    ...
                    if (this->badmatrix[k][j]) { continue; }
                    if (abs(this->cmatrix[i][k] - t) == 1 && this->cmatrix[i][k] > 0) {
                        
                        int a = min(k, j);
                        int b = max(k, j);
                        uint32_t mk = (a << 8) | b;
                        if (mp.find(mk) == mp.end()) {
                            mp.emplace(mk, 1);
                        } else {
                            mp[mk] += 1;
                        }
                        
                    } else if (abs(this->cmatrix[i][k] - t) > 1 && this->cmatrix[i][k] > 0) {
        ...
        for (auto&& i : mp) { vp.emplace_back(i.first, i.second); }
        sort(vp.begin(), vp.end(), [](const pair<uint32_t, int>& a, const pair<uint32_t, int>& b) { return a.second > b.second; });
        ...
        for (auto&& i : vp) {
            ...
            bool fff = true;
            for (int j = 0; j < this->N; ++j) {
                if (this->cmatrix[j][a] < 1) { continue; }
                if (this->cmatrix[j][b] < 1) { continue; }
                if (abs(this->cmatrix[j][a] - this->cmatrix[j][b]) > 1) {// !=
                    fff = false;
                    break;
                }
            }
            ...
            if (i.second > o && fff) {
                ...
                for (auto&& v : this->paths) {
                    if (a == v.a && b == v.b && v.c > 1) { ff = false; }
                    if (b == v.a && a == v.b && v.c > 1) { ff = false; }
                }
                ...
                if (ff) {
                    ...
                    if (f) {
                        ...
                        e.pop_back();
                        e.pop_back();
                        this->badmatrix[a][b] = true;
                        this->badmatrix[b][a] = true;
                        this->cmatrix[b][a] = -1;
                        this->cmatrix[a][b] = -1;
                    }
                }
                ...
                for (int k = 0; k < this->N; ++k) {
                    ...
                    if (abs(this->cmatrix[i][k] - t) == 1 && this->cmatrix[i][k] > 0) {
                        ...
                        for (int x = 0; x < this->N; ++x) {
                            if (this->cmatrix[x][j] < 1) { continue; }
                            if (this->cmatrix[x][k] < 1) { continue; }
                            if (abs(this->cmatrix[x][j] - this->cmatrix[x][k]) > 1) {// !=
                                fff = false;
                                break;
                            }
                        }
                        bool ff = true;
                        for (auto&& v : this->paths) {
                            if (k == v.a && j == v.b && v.c > 1) { ff = false; }
                            if (j == v.a && k == v.b && v.c > 1) { ff = false; }
                        }
    ...
    void disConnect2 () {
        for (int i = 0; i < this->N; ++i) {
            auto vn = this->connectedNode(i);
            for (int ji = 0; ji < vn.size(); ++ji) {
                int j = vn[ji];
                for (int k = 0; k < this->N; ++k) {
                    if (this->cmatrix[j][k] == -1) {
                        for (int oi = 0; oi < vn.size(); ++oi) {
                            int o = vn[oi];
                            this->cmatrix[o][k] = -1;
                            this->cmatrix[k][o] = -1;
                            this->badmatrix[o][k] = true;
                            this->badmatrix[k][o] = true;
                        }
                    }
                }
    ...
    vector<int> connectedNode (int x) {
        ...
        while (v.size() > 0) {
            ...
            v.pop_back();
            for (int i = 0; i < this->N; ++i) {
                if (this->cmatrix[t][i] > 0 && us.find(i) == us.end()) {
                    v.emplace_back(i);
                }
            }
        }
    ...
    int achievementCnt (vector<Edge>& e) {
        int r = 0;
        auto g = GraphDijkstra(this->N, e, true);
        for (auto&& i : this->paths) {
            if (i.c > 0) {
