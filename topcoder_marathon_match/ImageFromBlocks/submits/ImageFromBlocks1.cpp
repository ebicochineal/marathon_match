...
public:
    ...
    string placePiece(int pieceHeight, vector<int> piece, int timeLeft) {
        ...
        if ((float)this->aa / (float)(this->bh *this->bw) < 0.8) {
            dis = xx * ((float)this->discard / (float)this->maxDiscard);
        } else {
            dis = xx * ((float)this->aa / (float)(this->bh *this->bw));
        }
        
        
        if (bestscore > dis+0.1 || this->discard <= 0 || timeLeft < 500) {
            ...
            this->aa += this->polyominos[pmindex][0].pi.size();
        } else {
            this->discard -= 1;
        }
