...
public:
    ...
    string init(int imageHeight, int blockSize, int maxDiscard, vector<int> image) {
        ...
        // pdebug("B", this->blockSize);
        pdebug("D", (float)this->bh * this->bw / this->maxDiscard);
    ...
    string placePiece(int pieceHeight, vector<int> piece, int timeLeft) {
        ...
        for (int r = 0; r < 4; ++r) {
            for (int x = 0; x < this->bw; ++x) {
                float tcs, tps;
                this->put_test_score(pmindex, r, x, piece, tcs, tps);
                float tscore = tcs + tps;
                if (tscore > bestscore) {
                    bestscore = tscore;
                    bestx = x;
                    bestrot = r;
        ...
        float z = this->polyominos[pmindex][0].pi.size() * 0.5;
        
        float dis;
        // if (this->discard < 1000) { dis = z * 0.9; }
        // if (this->discard < 500 ) { dis = z * 0.8; }
        // if (this->discard < 250 ) { dis = z * 0.7; }
        // if (this->discard < 125 ) { dis = z * 0.5; }
        // if (this->discard < 64  ) { dis = z * 0.25; }
        // if (this->discard < 32  ) { dis = z * 0.1; }
        
        if (this->fillp < 0.8) {
            dis = z * ((float)this->discard / (float)this->maxDiscard);
        } else {
            dis = z * (this->fillp);
        }
        if (this->discard < 32) { dis *= 0.5; }
        if ((float)bestscore / this->polyominos[pmindex][0].pi.size() > dis/3 || this->discard <= 0 || timeLeft < 500) {
            if(!this->put(pmindex, bestrot, bestx, piece)){
                //pdebug(this->score, this->sw.get_milli_time(), timeLeft);
                pdebug(this->fillp, this->discard);
            }
            
            ret = to_string(bestrot) + " " + to_string(bestx);
            this->fill += this->polyominos[pmindex][0].pi.size();
            this->fillp = (float)this->fill / (float)(this->bh * this->bw);
    ...
    void put_test_score (int index, int rot, int sx, vector<int>& piece, float& tcs, float& tps) {
        const polyomino& mino = this->polyominos[index][rot];
        int mdown = 10000;
        
        tcs = 0; tps = 0;
        
        if (mino.w + sx > this->bw) { tps = -100; return; }
        for (int i = 0; i < mino.w; ++i) {
            const int col = this->bh - this->heightboard[sx+i] - mino.downs[i];
            if (col < mdown) { mdown = col; }
        }
        int p = this->bh - mdown;
        if (p + mino.h > this->bh) { tps = -100; return; }
        
        
        for (auto&& i : mino.pi) {
            tcs += this->calc_block_score(piece[i.second], sx+i.first.x, p+i.first.y);
        }
        
        for (int i = 0; i < mino.w; ++i) {
            tps -= ((p - mino.downs[i]) - this->heightboard[sx+i]) * 5;
        }
        ...
        for (int i = 0; i < mino.w; ++i) {
            ...
            tps -= ((p + mino.ups[i]) - minh) * max(min(this->fillp * 0.05f, 0.05f), 0.025f) * (this->maxDiscard < 80 ? 0.1 : 1);
            
        }
        
        vector<int> theightboard = this->heightboard;
        for (int i = 0; i < mino.w; ++i) {
            theightboard[sx+i] = mino.ups[i] + p + 1;
        }
        
        float sa = this->heightboard_score(this->heightboard);
        float sb = this->heightboard_score(theightboard);
        ...
        // int a, b, c;
        // a = sx-2 >= 0 ? this->heightboard[sx-2] : this->bh;
        // b = sx-1 >= 0 ? this->heightboard[sx-1] : this->bh;
        // c = p + mino.ups[0];
        // if (a-b > 0 && c-b > 0) {
        //     // tps -= min(a-b, c-b) * (this->fillp>0.5 ? 0.05 : 0.025);
        //     tps -= min(a-b, c-b) * (this->fillp>0.5 ? 0.05 : 0.025) * (this->maxDiscard < 80 ? 8 : 1);
        // }
        
        // a = sx + mino.w + 1 < this->bw ? this->heightboard[sx + mino.w + 1] : this->bh;
        // b = sx + mino.w < this->bw ? this->heightboard[sx + mino.w] : this->bh;
        // c = p + mino.ups[mino.w-1];
        // if (a-b > 0 && c-b > 0) {
        //     // tps -= min(a-b, c-b) * (this->fillp>0.5 ? 0.05 : 0.025);
        //     tps -= min(a-b, c-b) * (this->fillp>0.5 ? 0.05 : 0.025) * (this->maxDiscard < 80 ? 8 : 1);
        // }
    ...
    float heightboard_score (vector<int>& hb) {
        float r = 0;
        for (int i = 0; i < this->bw; ++i) {
            int a, b, c;
            a = i-1 >= 0 ? hb[i-1] : this->bh;
            b = hb[i];
            c = i+1 < this->bw ? hb[i+1] : this->bh;
            if (a-b > 0 && c-b > 0) {
                r -= min(a-b, c-b) * 0.05f;
            }
        }
        ...
        //     b = hb[i];
        //     c = i+1 < this->bw ? hb[i+1] : this->bh;
        //     if (a <= this->bh && b < this->bh && c <= this->bh) {
        //         r -= 0.05;
        //     }
        // }
