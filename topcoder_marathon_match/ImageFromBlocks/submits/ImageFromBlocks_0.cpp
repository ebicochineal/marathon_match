...
public:
    int bh, bw, x;
    int imageHeight, imageWidth, blockSize, maxDiscard;
    vector<int> image;
    vector<int> rimage;
    ...
    string init(int imageHeight, int blockSize, int maxDiscard, vector<int> image) {
        ...
        pdebug(this->blockSize, this->maxDiscard, this->sw.get_milli_time());
    ...
    string placePiece(int pieceHeight, vector<int> piece, int timeLeft) {
        ...
        if(!this->put(pmindex, bestrot, bestx, piece)){ pdebug(score); };
        string ret = to_string(bestrot) + " " + to_string(bestx);
    ...
    float put_test_score (int index, int rot, int sx, vector<int>& piece) {
        ...
        for (int i = 0; i < mino.w; ++i) {
            r -= ((p - mino.downs[i]) - this->heightboard[sx+i]) * 0.25;
        }
        int minh = 10000;
