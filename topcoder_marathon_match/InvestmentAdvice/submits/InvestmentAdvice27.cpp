...
public:
    ...
    void calc_accuracy (vector<int> advice, vector<int> recent) {
        ...
        for (int i = 0; i < this->size; ++i) {
            ...
            if (this->prevret[i] > 0) {
                ...
                if (diff2[i] > (this->cnt > 16 ? min(max(6, (int)(15 - ((this->accuracy[i] * 10)-0.6))*2 ), 16) : 10)) {
    ...
    vector<int> getInvestmentsB(vector<int> advice, vector<int> recent, int money, int timeLeft, int roundsLeft) {
        ...
        for (int i = 0; i < advice.size(); ++i) {
            ...
            if (this->accuracy[i] < 0.7) { adv[i] *= ( this->cnt > 16 ? 0.8 : 0.9); }
            if (this->accuracy[i] < 0.6) { adv[i] *= ( this->cnt > 16 ? 0.8 : 0.9); }
            if (adv[i] >= 0) {
