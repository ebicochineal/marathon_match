...
public:
    ...
    void calc_accuracy (vector<int> advice, vector<int> recent) {
        ...
        for (int i = 0; i < this->size; ++i) {
            ...
            diff[i] = ((double)recent[i] - (double)this->prevrec[i]) / d;
            diff2[i] = abs(diff[i] - this->prevadv[i]);
        }
        
        
        for (int i = 0; i < this->size; ++i) {
            if (diff2[i] > 20 - (this->prevadv[i] >= 0 ? 2 : 0)) {
                // x
            } else {
                // o
    ...
    vector<int> getInvestments(vector<int> advice, vector<int> recent, int money, int timeLeft, int roundsLeft) {
        ...
        for (int i = 0; i < advice.size(); ++i) {
            if (this->accuracy[i] > 0.9 && this->cnt > 8) { advice[i] *= 1.25; }
            if (this->accuracy[i] > 0.8 && this->cnt > 8) { advice[i] *= 1.25; }
            
            if (this->accuracy[i] > 0.7 && this->cnt > 16) { advice[i] *= 1.25; }
            ...
            if (this->accuracy[i] < 0.8 - min(this->cnt * 0.002, 0.3)) { advice[i] *= 0.9; }
            if (this->accuracy[i] < 0.7 - min(this->cnt * 0.002, 0.3)) { advice[i] *= 0.9; }
