...
public:
        ...
        while ((saTimeCurrent = sw.get_milli_time()) < saTimeEnd) {
            ...
            for (int i = 0; i < loop; ++i) {
                ...
                if (tscore > lastscore || (FORCE_NEXT)) {
                    lastscore = tscore;
                    // if (tmp == 2) { this->Remove(board, H, W); }
    ...
    // double DiffScore (vector< vector<int> > &board, double ascore, int tx, int ty, int c) {
    //     static const int dy[] = {-2, -1, 1, 2, 2, 1, -1, -2};
    //     static const int dx[] = {1, 2, 2, 1, -1, -2, -2, -1};
    //     int bcnt = 0;
    //     for (int z=0; z<8; ++z) {
