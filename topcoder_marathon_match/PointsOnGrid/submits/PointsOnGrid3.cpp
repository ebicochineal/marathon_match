...
public:
    ...
    vector<string> findSolution (int H, int W, int h, int w, int Kmin, int Kmax, vector<string> board) {
        ...
        for (int i = 1; i < 32; ++i) {
            vector< vector<int> > g2 = this->rgreedy(i);
            if (this->allScoreCheck(g2) > bestscore) {
                bestscore = this->allScoreCheck(g2);
                g = g2;
            }
        ...
        int gscore = this->allScoreCheck(g);
        if (gscore < 0) { pdebug("---"); }
        vector< vector<int> > iret = (gscore > bestscore ? g : best);
        bestscore = this->allScoreCheck(iret);
        int cnt = 0;
        while (this->sw.get_milli_time() < 500 && gscore > -1) {
            for (int i = 0; i < 1000; ++i) {
                int x = xrnd() % this->W;
                int y = xrnd() % this->H;
                if (iret[y][x] == 1) { continue; }
                iret[y][x] = 1;
                int score = this->allScoreCheck(iret);
                if (score > bestscore) {
                    bestscore = score;
                } else {
                    iret[y][x] = 0;
        ...
        //     }
        // }
        iret = this->SA(9800 - this->sw.get_milli_time() , iret, cnt);
        
        pdebug(cnt);
        pdebug(this->sw.get_milli_time());
        return this->toRet(iret);
    }
    
    vector< vector<int> > SA (int limit, vector< vector<int> > ret, int &cnt) {////
        StopWatch sw;
    ...
    vector< vector<int> > greedy (int n) {
        ...
        for (auto &i : mp) {
            ...
            if (cnt < this->Kmax-n) {
                ...
                for (auto &j : t) {
                    const mpos p = j.second;
                    if (this->iboard[p.y][p.x] < 2) { continue; }
                    ret[p.y][p.x] = 1;
                    minx = min(minx, p.x);
                    miny = min(miny, p.y);
                    maxx = max(maxx, p.x);
    ...
    vector< vector<int> > rgreedy (int n) {
        vector< vector<int> > ret(this->H, vector<int>(this->W, 0));
        vector< vector<int> > xxx(this->H, vector<int>(this->W, 0));
        ...
        for (int y = 0; y < this->H - (this->h - 1); ++y) {
            for (int x = this->W - (this->w - 2); x >= 0 ; --x) {
                mp.emplace_back(x, y);
            }
        }
