...
class TopCoderTesterQueue(threading.Thread):
    ...
    def run(self):
        #cmd = 'java -Xmx1024m -jar ' + self.jarpath + ' -exec ' + self.cmdpath + ' -seed ' + str(self.n) + ' -save ./image/' + str(self.n) + '.png'
        
        cmd = 'java -Xmx1024m -jar ' + self.jarpath + ' -exec ' + self.cmdpath + ' -seed ' + str(self.n)
        ...
        if sp in out:
            if '<cerr>' in out:
                self.result = (self.n, decimal.Decimal(out.split(sp)[1]), out.split(sp)[0].split('<cerr>')[1].split('</cerr>')[0])# out.split(sp)[0] # out.split(sp)[0].split('<cerr>')[1].split('</cerr>')[0]
            else:
                self.result = (self.n, decimal.Decimal(out.split(sp)[1]), '')
        else:
