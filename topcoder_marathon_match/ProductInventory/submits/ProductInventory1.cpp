...
public:
    ...
    vector<int> order(vector<int> yesterday) {
        ...
        if (this->day == 1) {
            ...
            for (int i = 0; i < ret.size(); ++i) {
                this->VOrder(i, max(this->sells[i] / 5, 3), ret);
            }
        } else {
            this->Day();
            this->Sell(yesterday);
            ...
            for (int i = 0; i < ret.size(); ++i) {
                ...
                if (this->items[i].size() < 1) {
                    this->VOrder(i, 1- this->items[i].size(), ret);
                }
                // if (this->items[i].size() < 5) {
                //     this->VOrder(i, max(yesterday[i], 5), ret);
                // } else {
                //     this->VOrder(i, yesterday[i], ret);
                // }
            }
