...
public:
    ...
    void Sell (vector<int> yesterday) {
        ...
        for (int i = 0; i < yesterday.size(); ++i) {
            if (yesterday[i] == this->items[i].size()) { this->rsells[i] += max(this->day / 20, 2); }
    ...
    vector<int> order(vector<int> yesterday) {
        ...
        for (int i = 0; i < yesterday.size(); ++i) {
            if (this->day == 0) {
                ...
                this->maxsell = max(this->maxsell, yesterday[i]/10);
                this->minsell = min(this->minsell, yesterday[i]/10);
            }
            this->sells[i] += yesterday[i];
            this->rsells[i] += yesterday[i];
            this->maxsell = max(this->maxsell, yesterday[i]);
            this->minsell = min(this->minsell, yesterday[i]);
        }
        ...
        if (this->day == 1) {
            ...
            this->Disposal();
            for (int i = 0; i < ret.size(); ++i) {
                int n = this->rsells[i] / (10 + this->day -1) + (int)ret.size()/(40+this->day);
        ...
        //if (this->day == 1) { cerr << "<cerr>" << this->maxsell << " " << this->minsell << "</cerr>"<< endl; }
        return ret;
    }
};
