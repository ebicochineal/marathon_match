...
public:
    ...
    vector<int> findSolution (int NM, int N, int E, vector<string> edges, int R, vector<string> routes) {
        ...
        for (int i = 0; i < E; i++) {
            ...
            this->edges2.emplace_back(e);
            this->edges3.emplace_back(e);
            this->redges.emplace_back(e);
        ...
        for (int i = 0; i < this->edges.size(); ++i) {
            int ea = this->edges[i].a;
            int eb = this->edges[i].b;
            int cost = this->edgeindex_costs[i] * 100;
            int point = this->edgeindex_points[i];
            int v1  = ( xxx.find(ea)!= xxx.end()) + ( xxx.find(eb)!=xxx.end());
            int v2  = (xxx2.find(ea)!=xxx2.end()) + (xxx2.find(eb)!=xxx2.end());
            int v3  = (xxx3.find(ea)!=xxx3.end()) + (xxx3.find(eb)!=xxx3.end());
            int v4  = (xxx4.find(ea)!=xxx4.end()) + (xxx4.find(eb)!=xxx4.end());
            
            cost = cost / (point + v1 * 3) - v2 * 3 - v3 - v4;
        ...
        for (int i = 0; i < this->edges.size(); ++i) {
            int ea = this->edges[i].a;
            int eb = this->edges[i].b;
            int cost = this->edgeindex_costs[i] * 20;
            int point = this->edgeindex_points[i];
            int v1  = ( xxx.find(ea)!= xxx.end()) + ( xxx.find(eb)!=xxx.end());
            int v2  = (xxx2.find(ea)!=xxx2.end()) + (xxx2.find(eb)!=xxx2.end());
            int v3  = (xxx3.find(ea)!=xxx3.end()) + (xxx3.find(eb)!=xxx3.end());
            ...
            cost = cost / (point + v1 * 2);
            this->edges2[i].cost = max(cost, 1);
        }
        ...
        for (int i = 0; i < this->edges.size(); ++i) {
            int ea = this->edges[i].a;
            int eb = this->edges[i].b;
            int cost = this->edgeindex_costs[i] * 1000;
            int point = this->edgeindex_points[i];
            int v1  = (xxx.find(ea)!=xxx.end())+(xxx.find(eb)!=xxx.end());
            int v2  = (xxx2.find(ea)!=xxx2.end())+(xxx2.find(eb)!=xxx2.end());
            int v3  = (xxx3.find(ea)!=xxx3.end())+(xxx3.find(eb)!=xxx3.end());
            ...
            cost = cost - point * 10 - v1 - v2 - v3 - v4;
            this->redges[i].cost = max(cost, 1);
        }
        ...
        bool upf = true;
        int upfcnt = 0;
        while (saTimeCurrent < saTimeEnd && dcnt < 5000) {
            ...
            if (fixf && dcnt > 1000) {
                ...
                for (int i = 0; i < this->edges.size(); ++i) {
                    ...
                    cost = cost - point*2 - v1*3 - v2*2 - v3 - v4;
                    this->edges[i].cost = max(cost, 1);
                }
                for (int i = 0; i < this->edges.size(); ++i) {
                    int ea = this->edges[i].a;
                ...
                for (int i = 0; i < this->edges.size(); ++i) {
                    ...
                    cost = cost -  point*2 - v1*2 - v2*2 - v3*2 - v4*2;
                    this->edges3[i].cost = max(cost, 1);
                }
        ...
        int ecnt = 0;
        int upcnt = 0;
        while (timer < 9600 && ecnt < 20000) {
            if (ecnt % m == 0) { timer = this->sw.get_milli_time(); }
            if (timer < 800 && cnt > 200) { m = 4; }
            if (timer < 800 && cnt > 500) { m = 8; }
            if (timer < 800 && cnt > 1000) { m = 16; }
            if (timer < 800 && cnt > 5000) { m = 32; }
        ...
        while (timer < 9600) {
            ...
            for (auto &i : this->routes) {
                ...
                if (lf) {
                    g.edge_update(this->update_edges(this->update_edges2(this->redges, tignore), usret));
                } else if (lrn == 0) {
                    g.edge_update(this->update_edges(this->update_edges2(this->edges2, tignore), usret));
                } else if (lrn == 1) {
                    g.edge_update(this->update_edges(this->update_edges2(this->edges3, tignore), usret));
                } else if (lrn == 2) {
                    if (xrnd() % 4 == 0) {
                        g.edge_update(this->update_edges(this->update_edges2(this->redges, tignore), usret));
                    } else {
                        g.edge_update(this->update_edges(this->update_edges2(this->edges, tignore), usret));
                    }
                } else {
                    g.edge_update(this->update_edges(this->update_edges2(this->edges, tignore), usret));
                }
