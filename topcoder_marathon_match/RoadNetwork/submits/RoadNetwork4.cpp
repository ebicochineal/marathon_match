...
public:
    ...
    vector<int> findSolution (int NM, int N, int E, vector<string> edges, int R, vector<string> routes) {
        ...
        for (int i = 0; i < E; i++) {
            ...
            if (a < b) {
                p = mpos(a, b);
                // e = edge(a, b, cost);
                e = edge(a, b, cost*100/(point+(xxx.find(a)!=xxx.end())*5+(xxx.find(b)!=xxx.end())*5));
                
            } else {
                p = mpos(b, a);
                // e = edge(b, a, cost);
                e = edge(b, a, cost*100/(point+(xxx.find(a)!=xxx.end())*5+(xxx.find(b)!=xxx.end())*5));
            }
            this->gmin = min(cost*100/(point+(xxx.find(a)!=xxx.end())*5+(xxx.find(b)!=xxx.end())*5), this->gmin);
        ...
        //     if ((double)i.point/abs(i.a-i.b) < 2) {
        //         continue;
        //     }
        ...
        for (auto &i : this->routes) {
            g = graph_dijkstra(N, this->update_edges(this->edges, usret));
    ...
    vector<edge> update_edges (vector<edge> edges, unordered_set<int> usret) {
        for (auto &i : usret) {
            edges[i].cost = 1;
        }
        return edges;
