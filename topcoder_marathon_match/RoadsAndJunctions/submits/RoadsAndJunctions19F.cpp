...
inline float distance (const int &ax, const int &ay, const int &bx, const int &by) {
    // return hypot(ax - bx, ay - by);
    return sqrt((ax - bx) * (ax - bx) + (ay - by) * (ay - by));
}
...
public:
    ...
    vector<int> buildJunctions (int S, vector<int> cities, float junctionCost, float failureProbability) {
        vector<int> ret;
        S += 1;
        this->S = S;
        this->init_distmap();
        ...
        for (int i = 0; i < S * S; ++i) {
            int x = i % S;
            int y = i / S;
            if (this->bmap2[i] == 1) { this->bp.emplace_back(x, y); }
        }
    ...
    vector<int> buildJunctionsA (int S, vector<int> cities, float junctionCost, float failureProbability) {
        //this->sw = StopWatch();
        vector<int> ret;
        ...
        float iscore = this->all_distance(this->cities_graph, vps);
        float score = iscore;
        int cnt = 0;
        int tnt = 0;
        vector<int> putpos;
        ...
        int nx = (5126 * 20) / this->NC;
        float jc = junctionCost * (1.0 + failureProbability);//;
        score -= jc;
        int scnt = 0;
        while(timer < 9500){
            ...
            for (int z = 0; z < loop; ++z) {
                ...
                int p = this->bp[a].first + this->bp[a].second * this->S;
                if (vf[p] == 0) { continue; }
                auto tvps = this->add_vps(vps, p);
                auto g = this->ip_to_graph(tvps);
                float tscore = this->all_distance(g, tvps);
                if (tscore < score) {
                    score = tscore;
                    tpos = p;
                }
        ...
        for (int k = 0; k < putpos.size(); ++k) {
            ...
            if (putpos[k] > -1) {
                ...
                if ((score - tscore) > junctionCost * 3 && failureProbability > 0.1) {
                ...
                if ((score - tscore) > junctionCost * 4 && failureProbability > 0.2) {
                    int x = putpos[k] % this->S;
                    int y = putpos[k] / this->S;
    ...
    void init_distmap () {
        this->distmap = vector< vector<float> >(2048, vector<float>(2048, 2000));
    ...
    float tdistance (const int ax, const int ay, const int bx, const int by) {
    ...
    vector<int> union_pos(vector<int> &a) {
        ...
        for (int i = 0; i < p.size(); ++i) {
            ...
            for (int j = i+1; j < p.size(); ++j) {
                int bx = p[j] % this->S;
                int by = p[j] / this->S;
                float d = this->tdistance(ax, ay, bx, by);
                if (d < 8) {
                    sx += bx;
                    sy += by;
                    cnt += 1;
    ...
    inline float all_distance (const vector< pair<int, int> > &g, const vector<int> &vps) {
        float d = 0;
        int ax, ay, bx, by;
        for (auto &i : g) {
        ...
        while (s.size() < ip.size()) {
            ...
            for (int i = 0; i < s.size(); ++i) {
                ...
                for (auto& m : mip) {
                    ...
                    const float tdist = this->tdistance(sx, sy, px, py);
                    if (tdist < dist) {
                        dist = tdist;
                        ii = s[i].first;
                        mm = m;
...
int main() {
    ...
    vector<int> cities(C);
    getVector(cities);
    float junctionCost, failureProbability;
