...
public:
    ...
    vector<int> buildJunctions(int S, vector<int> cities, double junctionCost, double failureProbability) {
        ...
        //     }
        // }
        vector< pair<int, int> > cities_graph = this->ip_to_graph(this->ip_cities);
        vector<int> rcnts =  this->road_cnt_pos(cities_graph);
        for (auto &i : this->ip_cities) {
            if (rcnts[i.first] < 2) { continue; }
