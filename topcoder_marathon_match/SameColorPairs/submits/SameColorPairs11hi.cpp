...
public:
    ...
    vector<string> removePairs(vector<string> board) {
        ...
        while (sw.get_milli_time() < 2500 && this->bestret.size() < H * W / 2) {
            this->remove_cellsv();
        ...
        while (sw.get_milli_time() < 6000 && this->bestret.size() < H * W / 2) {
            this->remove_cellsvr();
            if (this->ret.size() > this->bestret.size()) {
                this->bestret = this->ret;
                ...
                if (this->ret.size() > H*W/2 - (56 - rets.size()) && rets.size() < 32) {
        ...
        for (int i = 0; i < min(8, (int)rets.size()); ++i) {
            this->ret_rewind(16, rets[rets.size()-i-1]);
        }
