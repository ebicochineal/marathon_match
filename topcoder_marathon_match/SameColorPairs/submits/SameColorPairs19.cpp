...
public:
    ...
    vector<string> removePairs(vector<string> board) {
        ...
        vector<int> re = {H*W/8, H*W/8, H*W/16, H*W/24, H*W/32, H*W/40, 32, 32, 32};
        
        for (int n = 1; n < 9; ++n) {
            while (swtime < 615 * n && this->bestret.size() < H * W / 2) {
        ...
        this->bestret_rewind(H*W/2);
        for (int n = 1; n < 9; ++n) {
            while (swtime < 615 * n + 4920 && this->bestret.size() < H * W / 2) {
                if (cnt % 20 == 0) { swtime = sw.get_milli_time(); }
                this->remove_cellsvr();
                if (this->ret.size() > this->bestret.size()) {
                    this->bestret = this->ret;
