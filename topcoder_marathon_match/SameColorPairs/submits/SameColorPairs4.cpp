...
public:
    ...
    inline bool rect_first_remove (const int ax, const int ay, const int mrx, const int mdy, const vector<string> &board, int &px, int &py) {
        // int mlx = 0;
        // for (int i = 0; i <= mdy; ++i) {
        ...
        for (int i = ay; i <= mdy; ++i) {
            for (int j = 0; j <= mrx; ++j) {
                if (board[i][j] == c && (j != ax || i != ay) && this->rect_is_remove(ax, ay, j, i, board)) {
                    py = i;
                    px = j;
