...
public:
    ...
    vector<int> create(int H, vector<int> pixels, int N) {
        ...
        for (int i = 0; i < 64; ++i) {
            vector<XYC> tmp2 = tmp;
            this->subtractiveColor(4, 24 + i*2, tmp2);
            int64_t bscore = this->allScore(4, tmp2);
            if (bscore < bestscore) {
                bestscore = bscore;
                best = tmp2;
            }
            if (i>0 && i%4==0 && this->sw.get_milli_time() > 18000) { break; }
        }
    ...
    void subtractiveColor(const int o, const int q, vector<XYC> &t) {
