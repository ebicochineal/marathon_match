...
public:
    ...
    vector<int> create(int H, vector<int> pixels, int N) {
        ...
        vector<int> bestscores = this->fixColorScores(4, best);
        while (1) {
            if (cnt>0 && cnt%4==0 && this->sw.get_milli_time() > 12000) { break; }
            cnt += 1;
            vector<XYC> a = best;
            for (int p = 0; p < N; ++p) {
            ...
            vector<int> tscores = this->fixColorScores(4, a);
            vector<XYC> b = best;
            for (int p = 0; p < N; ++p) {
            ...
            tscores = this->fixColorScores(4, b);
            int64_t tscore = 0;
        ...
        bestscore = this->fixColorScore(4, best);
        
        unordered_set<mpos> us;
        for (auto &i : best) { us.emplace(i.x, i.y); }
        int  limit = 12000 + (N < 100 ? 6000 : 0);
        while (1) {
        ...
        for (int i = 0; i < 32; ++i) {
            tmp2 = tmp;
            this->subtractiveColor0(4, 24 + i*2, i, tmp2);
            ...
            // tmp2 = tmp;
            // this->subtractiveColor1(4, 24 + i*2, i, tmp2);
            // bscore = this->allScore(4, tmp2);
            // if (bscore < bestscore) {
            //     bestscore = bscore;
            //     best = tmp2;
            // }
            // tmp2 = tmp;
            // this->subtractiveColor2(4, 24 + i*2, i, tmp2);
            // bscore = this->allScore(4, tmp2);
            // if (bscore < bestscore) {
            //     bestscore = bscore;
            //     best = tmp2;
            // }
            // tmp2 = tmp;
            // this->subtractiveColor3(4, 24 + i*2, i, tmp2);
            // bscore = this->allScore(4, tmp2);
            // if (bscore < bestscore) {
            //     bestscore = bscore;
            //     best = tmp2;
            ...
            if (this->sw.get_milli_time() > 18000 + (N < 100 ? 1000 : 0)) { break; }
        }
