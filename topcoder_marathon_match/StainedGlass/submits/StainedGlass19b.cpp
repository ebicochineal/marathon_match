...
public:
    ...
    vector<int> create(int H, vector<int> pixels, int N) {
        ...
        int64_t bestscore = this->fixColorScore(1, best);
        
        for (int i = 0; i < 640 * (N < 150 ? 1 : 0); ++i) {
            vector<XYC> a;
            this->testPut2r(this->gridNum(), i, a);
            int64_t ascore = this->fixColorScore(1, a);
            if (ascore < bestscore) {
                bestscore = ascore;
                best = a;
            }
        ...
        bestscore = this->fixColorScore(1, best);
        vector<int> bestscores = this->fixColorScores(1, best);
        while (1) {
            if (cnt>0 && cnt%4==0 && this->sw.get_milli_time() > 14000) { break; }
            cnt += 1;
            vector<XYC> a = best;
            for (int p = 0; p < N; ++p) {
            ...
            vector<int> tscores = this->fixColorScores(1, a);
            vector<XYC> b = best;
            for (int p = 0; p < N; ++p) {
            ...
            tscores = this->fixColorScores(1, b);
            int64_t tscore = 0;
        ...
        while (1) {
            ...
            best[p].x += vx;
            best[p].y += vy;
            int64_t tscore = this->fixColorScore(1, best);
            if (tscore < bestscore) {
                bestscore = tscore;
