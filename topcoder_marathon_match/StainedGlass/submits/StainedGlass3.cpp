...
public:
    ...
    vector<int> create(int H, vector<int> pixels, int N) {
        ...
        vector<XYC> a;
        this->testPut(this->gridNum(), a);
        this->fixColor(a);
        this->subtractiveColor(a);
        int64_t ascore = this->allScore(a);
        
        vector<XYC> b;
        this->testPut2(this->gridNum(), b);
        this->fixColor(b);
    ...
    void fixColor2(vector<XYC> &t, unordered_map<int, vector<int> > &cols) {
        ...
        for (auto &i : cols) {
            ...
            array<int, 255> tr = array<int, 255>();
            array<int, 255> tg = array<int, 255>();
            array<int, 255> tb = array<int, 255>();
            for (auto &j : i.second) {
                for (int k = 0; k < 255; ++k) {
                    tr[k] += rcs[j][k];
                    tg[k] += gcs[j][k];
