...
public:
    ...
    vector<int> create(int H, vector<int> pixels, int N) {
        ...
        int cnt = 0;
        while (this->sw.get_milli_time() < 16000) {
            vector<XYC> b;
    ...
    void testPut2 (int n, int m, vector<XYC> &t) {
        for (int y = 0; y < n; ++y) {
            for (int x = 0; x < n; ++x) {
                t.emplace_back((this->W / n) * x + (this->W/n/2) + (y%2<1?(this->W/n)/2:0), xrnd()%(min(32, this->W/n-1)) + (this->H / n) * y + (this->H/n/2), 0);
            }
        }
    ...
    void subtractiveColor(const int o, vector<XYC> &t) {
