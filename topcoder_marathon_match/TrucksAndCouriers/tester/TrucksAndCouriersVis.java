...
public class TrucksAndCouriersVis {
  ...
  int[][][] warehouses = new int[101][101][50];
  int[][][] customers = new int[101][101][50];
  ...
  private void generateTestCase(long seed) {
    ...
    numItems = r.nextInt(41) + 10;/////////////////////////////////////////////
    
    numCustomers = r.nextInt(81);
    ...
    for (int i = 0; i < numCustomers; i++) {
      customerX[i] = r.nextInt(101);
      customerY[i] = r.nextInt(101);
    ...
    for (int i = 0; i < numItems; i++) {
      ...
      int[] c = new int[3];
      for (int j = 0; j < itemCount[i]; j++) c[r.nextInt(3)]++;
      for (int j = 0; j < 3; j++) {
        if (c[j] == 0) continue;
        wwx.add(wx[w[j]]);
        wwy.add(wy[w[j]]);
  ...
  private boolean checkXY(int startX, int startY, int destX, int destY) {
    ...
    if (destX < 0 || destX > 100) {
      failMessage += "Invalid destX: " + destX + "\n";
      return false;
    }
    if (destY < 0 || destY > 100) {
