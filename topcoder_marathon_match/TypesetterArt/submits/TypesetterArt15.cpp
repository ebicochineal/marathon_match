...
public:
    int init(int nRenders, int nFonts, int precision, int width, int height, vector<int> image) {
        ...
        this->ppv = vector< vector<int> >(height, vector<int>(width, 0));
        int index = 0;
        for (int k = 0; k < 4; k++) {
        ...
        for (int i = 0; i < nFonts; ++i) {
            if (precision == 1) {
                ...
                this->fonts.emplace_back(" l|.8T-[]:'ij@", i, 8);
                this->fonts.emplace_back(" l|.8T-[]:'ij@", i, 9);
                this->fonts.emplace_back(" l|.8T-[]:'ij@", i, 10);
                this->fonts.emplace_back(" l|.8T-[]:'ij@", i, 8);
            }
        }
    ...
    inline double diffScore (const E512Font &f, int x, int y, int k, int z) {
        ...
        if ( ! (y < 0 || x < 0 || y >= this->height || x >= this->width) ) {
            if (y+f.h >= this->height || x+f.w >= this->width) {
                for (auto &i : f.pos) {
                    ...
                    if (this->ppv[p.y][p.x] == 0 && tmp > 0) {
                        um.emplace_back(p);
                    }
                    this->ppv[p.y][p.x] += tmp;
                }
            } else {
                for (auto &i : f.pos) {
                    const int px = x + i.x;
        ...
        for (auto &i : um) {
            ...
            if (n != 0) {
                dSum = actualSum + this->ppv[i.y][i.x];
                originalSum /= n * 255;
                actualSum /= n * 255;
                dSum /= n * 255;
    ...
    void initrender (int limit) {
        ...
        int cnt = 0;
        for (int i = 0; i < 256; ++i) {
            if (this->sw.get_milli_time() > limit - 1500) { break; }
        ...
        this->prerender2(score, ret, bestn, limit);
        if (score > this->bestscore) {
            this->bestscore = score;
            this->bestret = ret;
        }
    ...
    void prerender (double &score, vector< vector<int> > &lret, int n, int limit) {
        ...
        for (int p = 0; p < this->nRenders; ++p) {
            ...
            int z = 255;
            if (this->precision == 1) { z = 40 + xrnd() % 200; }
            int k = (p+n) % 4;
            ...
            while (y < this->height) {
                ...
                if (p > o) {
                    ...
                    while (x < this->width) {
                        int bestindex = 0;
                        double bestscore = 0;
                        for (int i = 1; i < f.fonts.size() - 3; ++i) {
                            const double tscore = this->diffScore(f.fonts[i], x, y, k, z);
                            if (tscore > bestscore) {
                                bestscore = tscore;
                                bestindex = i;
                            }
                    ...
                    while (x < this->width) {
                        ...
                        double bestscore = 0;
                        for (int i = 1; i < f.fonts.size() - 3; ++i) {
                            const double tscore = this->diffScore2(f.fonts[i], x, y, k, z);
                            if (tscore > bestscore) {
                                bestscore = tscore;
                                bestindex = i;
                            }
    ...
    void prerender2 (double &score, vector< vector<int> > &lret, int n, int limit) {
        lret = vector< vector<int> >();
        this->kyxtmp = vector< vector< vector<int> > >(4, vector< vector<int> >(height, vector<int>(width, 0)));
        int time = 0;
        int o = this->nRenders - this->nRenders % 4;
