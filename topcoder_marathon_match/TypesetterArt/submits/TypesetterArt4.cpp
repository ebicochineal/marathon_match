...
public:
    int init(int nRenders, int nFonts, int precision, int width, int height, vector<int> image) {
        ...
        for (int i = 0; i < nFonts; ++i) {
            this->fonts.emplace_back(" |.@WT", i, 8);
            this->fonts.emplace_back(" l.@WT", i, 9);
            this->fonts.emplace_back(" |.@WT", i, 10);
            this->fonts.emplace_back(" l.@WT", i, 11);
        }
    ...
    void initrender () {
        ...
        double bestscore = 0;
        for (int i = 0; i < 32 / this->nRenders; ++i) {
            if (this->sw.get_milli_time() > 7000) { break; }
