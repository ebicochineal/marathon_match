...
public:
    int init(int nRenders, int nFonts, int precision, int width, int height, vector<int> image) {
        ...
        for (int i = 0; i < nFonts; ++i) {
            this->fonts.emplace_back(" |.@T", i, 8);
            this->fonts.emplace_back(" l.@T", i, 9);
            this->fonts.emplace_back(" |.@T", i, 10);
            this->fonts.emplace_back(" l.@T", i, 11);
        }
    ...
    vector<int> render (int iteration, int remainingMillis) {
        if (iteration == 0) { this->initrender(); }
        // int z = 255;
        // // if (this->nRenders < 9) { z = 150; }
        // if (this->precision == 1) { z = 80; }
        // int k = iteration % 4;
    ...
    void initrender () {
        ...
        for (int i = 0; i < 80; ++i) {
            if (this->sw.get_milli_time() > 8500) { break; }
            this->prerender(score, ret, i);
    ...
    void prerender (double &score, vector< vector<int> > &lret, int n) {
        lret = vector< vector<int> >();
        this->kyxtmp = vector< vector< vector<int> > >(4, vector< vector<int> >(height, vector<int>(width, 0)));
        for (int p = 0; p < this->nRenders; ++p) {
            if (this->sw.get_milli_time() > 8500 && this->bestscore > 0) {
                score = 0;
                return;
            }
            int z = 255;
            ...
            E512Fonts f = this->fonts[(p / 4) % this->nFonts];
            vector<int> ret = vector<int> { f.id, f.size, k, z };
            int y = 0; int x = 0;
            while (y < this->height) {
                x = 0;
