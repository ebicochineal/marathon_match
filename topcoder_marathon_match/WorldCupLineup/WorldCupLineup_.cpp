...
public:
    ...
    int createCurrentLineup (array<int, 10> &poss, array<bool, 10> &cords, array<int, 30> &calcAgg) {
        ...
        for (int i = 0; i < 10; ++i) {
            if (poss[i] >= this->member[i].size()) { continue; }
            uses |= 1 << this->member[i][poss[i]];
            Player p = this->players[this->member[i][poss[i]]];
            if (this->member[i][0] == F) {
                atk += p.atk * 2;
            } else if (this->member[i][0] == D) {
                def += p.def * 2;
            } else {
                atk += p.atk;
                def += p.def;
            }
            calcAgg[this->member[i][poss[i]]] = p.agg;
            positions[this->member[i][poss[i]]] = this->member[i][0];
        }
        for (auto &i : this->groups) {
    ...
    void processLineup (array<int, 10> &poss, array<bool, 10> &cords, array<int, 30> &calcAgg, int n) {
        if (calcAgg[this->member[n][poss[n]]]) { poss[n] += 1; }
    ...
    int Simulate (int n) {
        ...
        this->processLineup(poss, cords, calcAgg, n);
    ...
    void InitRefrain (vector<int> positions) {
        ...
        for (int i = 0; i < 30; ++i) {
            ...
            this->member[mi].emplace_back(positions[i]);
            this->member[mi].emplace_back(i);
            mi += 1;
        ...
        for (int k = 0; k < 3; ++k) {
            for (int j = 0; j < 10; ++j) {
                ...
                if (index > -1) {
                    this->member[j].emplace_back(index);
                    positions[index] = 'x';
            ...
            for (int k = 0; k < 2; ++k) {
                ...
                for (int i = 0; i < loop; ++i) {
                    ...
                    // {
                        int a = xrnd() % 10;
                        int b = xrnd() % 10;
                        int c = xrnd() % (this->team.member[a].size()-2);
                        int d = xrnd() % (this->team.member[b].size()-2);
                        
                        swap(this->team.member[a][c+2], this->team.member[b][d+2]);
                ...
                for (int i = 0; i < loop; ++i) {
                    {
                        ...
                        this->team.member[b].emplace_back(this->team.member[a][this->team.member[a].size()-1]);
                        this->team.member[a].pop_back();
                        int tscore = this->team.ReSimulate();
                        ...
                        if (tscore > lastscore || (FORCE_NEXT)) {
                            ...
                            this->team.member[a].emplace_back(this->team.member[b][this->team.member[b].size()-1]);
                            this->team.member[b].pop_back();
                        }
                    }
