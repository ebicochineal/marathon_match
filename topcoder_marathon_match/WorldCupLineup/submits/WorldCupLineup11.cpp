...
public:
    ...
    int createCurrentLineup (array<int, 10> &poss, array<bool, 10> &cords, array<int, 30> &calcAgg) {
        ...
        int atk = 0;
        int def = 0;
        for (int i = 0; i < 30; ++i) { calcAgg[i] = 0; }
        for (int i = 0; i < 10; ++i) {
    ...
    int ReSimulate () {
        ...
        return ret / 128 - abs(maxret - minret) / 16 + maxret / 16;
    }
...
public:
    ...
    vector<string> selectPositions(vector<string> players, vector<string> groups) {
        ...
        //     cerr << print(i) << endl;
        // }
        // cerr << print(this->team.ToRet(vi)) << endl;
