...
public:
    ...
    Group (string a, string b) {
        ...
        if (this->agg  > 15) { this->atk -= 6; }
        if (this->agg  > 15) { this->def -= 6; }
        if (this->agg  > 20) { this->atk -= 8; }
        if (this->agg  > 20) { this->def -= 8; }
    }
};
...
public:
    ...
    static vector<string> ToRet (Team t, vector<int> positions) {
        ...
        vector< pair<int, int> > index_agg;
        int cnt = 0;
        for (int i = 0; i < 30; ++i) {
            if (positions[i] == '-') { continue; }
            ret.emplace_back(print((char)positions[i], i));
            reti.emplace_back(i);
            index_agg.emplace_back(cnt, t.players[i].agg);
            cnt += 1;
        }
        
        sort(index_agg.begin(), index_agg.end(), [](const pair<int, int> &a, const pair<int, int> &b) { return a.second > b.second; });
        
        // for (int j = 0; j < 10; ++j) {
        for (auto &ia : index_agg) {
            int j = ia.first;
            ...
            if (ret[j][0] == 'F') {
                ...
                for (int i = 0; i < 30; ++i) {
                    if (positions[i] != '-') { continue; }
                    // if (t.players[reti[j]].agg < 5) { continue; }
